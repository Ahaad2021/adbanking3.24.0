...
$mnt_limit = 0;
$now = date('Y-m-d');
if ($data_agc['mnt_depot_ewallet_max_journalier'] > 0) {
    if ($data_client['date_depot_limit_ewallet_jounalier'] != $now || $data_client['date_depot_limit_ewallet_jounalier'] == NULL) {
        //update date et limit dans table ad_cli
        //arg type_transaction 1 (retrait), 2 (depot)
        $update_mnt_limit = updateMntLimiteEwallet($global_id_client, $data_agc['mnt_depot_ewallet_max_journalier'], $now, 2);
        if ($update_mnt_limit->errCode !== NO_ERR) {
            $MyErr = new ErrorObj('ERR_MISE_A_JOUR_MNT_LIMIT');
        }
        //set variable $mnt_limit = nouvaeu limit
        $mnt_limit = $data_agc['mnt_depot_ewallet_max_journalier'];
        $mnt_rest = $mnt_limit - $montant;
    } else {
        //set variable $mnt_limit = nouveau actuel
        $mnt_limit = $data_client['mnt_depot_limit_ewallet_jounalier'];
        $mnt_rest = $mnt_limit - $montant;
    }
}
/*
...
elseif ($montant <= 0) {
  ...
  $out = 1;
}elseif($montant> $mnt_limit && !empty($mnt_limit)) {
    $MyErr = new ErrorObj('ERR_MONTANT_DEPASSE_LIMIT_DEPOT_JOURNALIER');
    $out = 1;
}
else
{
  ...
  if ($erreur->errCode != NO_ERR)  { // Erreur check depot
    ...
    if ($erreur->errCode === NO_ERR) {
      ...
      //Pending en attendant evol pour les salariés/ commercants
          ...
          // $rembourse_erreur = null;
          // if(isMobileLending()) {
          //     $montant_remboursement = $mnt_reel;
          //     if (!empty($err->datas)) {
          //         $montant_tarif = $err->datas;
          //         $montant_remboursement -= $montant_tarif[0]["montant"];
          //     }
          ...
          //             'success' => false,
          //             'datas' => array(
          //                 'msg' => 'ERROR in rembourseMontantDepot:' . $rembourse_erreur->param,
          //             ),
          //         );
          //     }
          // }

          // if(empty($rembourse_erreur) || ($rembourse_erreur->errCode == NO_ERR)) {
...
//                  'success' => false,
//                  'datas' => array(
//                      'msg' => 'ERROR in updateMntLimiteEwallet:'. $MyErr->param,
//                  ),
//              );
//          }
