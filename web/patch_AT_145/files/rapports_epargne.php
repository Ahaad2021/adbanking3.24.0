	...
	require_once 'lib/misc/excel.php';
 	...
 	/*{{{ Era-5 : Comptes inactifs : choix du seuil en jours */
 	    ...
 	    if ($global_nom_ecran == "Era-5") {
            ...
            $html->addField("date_rapport", _("Date du rapport"), TYPC_DTE);
            ...
            $html->addField("etat", _("Etat"), TYPC_LSB);
            $html->setFieldProperties("etat", FIELDP_HAS_CHOICE_AUCUN, FALSE);
            $html->setFieldProperties("etat", FIELDP_ADD_CHOICES, array('1' => 'En jours', '2' => 'Inactif', '3' => 'Dormant'));

            $html->addField("nbre_jours", _("Comptes inactifs / dormants depuis (en jours)"), TYPC_INT);
			...
			$html->setFieldProperties("prodEpargne", FIELDP_HAS_CHOICE_TOUS, true);
 	        ...
 	        $js = "
 	        	$('[name=HTML_GEN_LSB_etat]').change(function(){
 	        		if($(this).val() == 2 || $(this).val() == 3){
 	        			$('[name=nbre_jours]').prop('readonly', true);
 	        		}else{
 	        			$('[name=nbre_jours]').prop('readonly', false);
 	        		}
 	        	});
 	        ";

            $html->addJS(JSP_FORM, "js_disabled", $js);
 	        $html->buildHTML();
 	        echo $html->getHTML();
 	    }
 	/*}}}*/
 	...
 	/*{{{ Era-6 et Era-27 : Comptes inactifs : impression ou export CSV */
 	    ...
 	    if ($global_nom_ecran == "Era-6" || $global_nom_ecran == "Era-27") {
			...
			if($prodEpargne != 0) { //produit epargne pas egale a "TOUS"
 	        ...
 	        $lignesCptesInactifs = getLignesCptesInactifs($nbre_jours, $produit, $gest, $etat, $date_rapport);
 	        if ($lignesCptesInactifs == NULL) { // Aucun compte sélectionné
 	            ...
 	            if ($global_nom_ecran == "Era-6") {
                    $lignesCptesInactifs['date_rapport'] = $date_rapport;
                    $lignesCptesInactifs['etat'] = $etat;
...
//		$myForm->setFieldProperties("critere_ep", FIELDP_ADD_CHOICES, $choix_ep);
	...
	$tranche_data = get_data_liste_epargne($list_agence, $SESSION_VARS['critere_ep'], $SESSION_VARS['type_epargne'], $SESSION_VARS['nbre_ligne_rapport'],$offset,$SESSION_VARS['date_rapport'], $SESSION_VARS['zone']);
