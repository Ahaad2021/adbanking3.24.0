...
function xml_comptes_inactifs($DATA, $nbre_jours, $produit, $export_csv = false) {
  ...
  */
  ...
  if (is_array($DATA)) {
    ...
    $list_criteres = array ();
    $list_criteres[_("Date du rapport")] = $DATA['date_rapport'];
    ...
    $list_criteres[_("Etat")] = ($DATA['etat'] == 1)?'En jours':(($DATA['etat'] == 2)?'Inactif':'Dormant');

    gen_criteres_recherche($header_contextuel, $list_criteres);

    $infos_synthetiques = $header_contextuel->new_child("infos_synthetiques", "");
    $infos_synthetiques->new_child("etat_compte_definition", ($DATA['etat'] == 1)?"Comptes inactifs / dormants":(($DATA['etat'] == 2)?"Comptes inactifs":"Comptes dormants"));
...
function get_data_liste_epargne($list_agence, $id_prod_ep, $type_epargne, $limit,$offset,$date_rapport, $zone) {
  global $adsys;
  ...
  foreach($list_agence as $key_id_ag =>$value) {
    ...
    array_merge($array_devise,$devises);
    // Récupère les données
    $result = get_liste_epargne($critere, $limit,$offset,$date_rapport, $type_epargne, $zone);
    if ($result->errCode != NO_ERR) {
      return $result;
    } else {
      $data = $result->param;
