...
function prelevement_frais_tenue_cpt() {
  ...
  if ($freq > 0) {
    ...
    $sql = "SELECT * FROM PreleveFraistenueCptbnr($freq, '$date_total', $type_operation, '".SENS_CREDIT."', '$appli_frais_gs');";
    $result = $db->query($sql);
...
function traiteComptesDormants($_date_batch) {
    ...
    $sql_setdormant = " SELECT * FROM setdormant('$_date_batch',$global_id_agence);";
    $result_setdormant= $db->query ($sql_setdormant);

    if (DB::isError ( $result_setdormant )) {
        $dbHandler->closeConnection ( false );
        erreur ( "setdormant", "Une erreur est survenue lor des desactivation des compte");
    }

    $dormant_count = $result_setdormant->fetchrow ( DB_FETCHMODE_ASSOC );
	$count = $result->param[0]['traitecomptesdormants'];
	$dbHandler->closeConnection ( true );
	affiche ( sprintf ( _ ( "OK (%s Comptes dormants ont été bloqués )" ), $count + $dormant_count['setdormant']), true );
	decLevel ();
...
/**
 * Fonction de gestion des comptes inactifs
 *
 * @param DATE $_date_batch
 * @return $objError
 */
function traiteComptesInactifs($_date_batch) {
    global $dbHandler, $error;
    global $global_id_agence;
    global $global_mouvements_epargne;

    affiche ( _ ( "Démarre le traitement des comptes inactifs..." ) );
    incLevel ();
    $db = $dbHandler->openConnection ();
    $sql = " SELECT * FROM traiteComptesInactifs('$_date_batch',$global_id_agence);";
    $result = $db->query ( $sql );

    if (DB::isError ( $result )) {
        $dbHandler->closeConnection ( false );
        erreur ( "traiteComptesInactifs", $result->getUserInfo () );
    }

    $count = $result->numrows ();
    while ( $row = $result->fetchrow ( DB_FETCHMODE_ASSOC ) ) {
        $myErr = deactiverCompteInactif ( $row, $global_mouvements_epargne );
        if ($myErr->errCode != NO_ERR) {
            $dbHandler->closeConnection ( false );
            return $myErr;
        }
    ...
    // $count = $result->param[0]['traitecomptesdormants'];
    $dbHandler->closeConnection ( true );
    affiche ( sprintf ( _ ( "OK (%s Comptes inactifs ont été bloqués )" ), $count ), true );
    decLevel ();
    affiche ( _ ( "Traitement des comptes inactifs terminé" ) );
    return $result;
}
...
function traite_epargne() {
 ...
 $myErr = traiteComptesDormants($date_total);
  ...
  $myErr = traiteComptesInactifs($date_total);
  if ($myErr->errCode != NO_ERR)
    signalErreur(__FILE__,__LINE__,__FUNCTION__, sprintf(_("Erreur lors des traitements COMPTES INACTIFS, code %s, param '%s'"), $myErr->errCode, $myErr->param));
