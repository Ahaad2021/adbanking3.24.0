...
function getLignesCptesInactifs($nbre_jours, $produit, $gestionnaire = 0, $etat = null, $date_rapport = null) {
  /*
  ...
  */
  ...
  $date_mask = date_create_from_format('d/m/Y', $date_rapport);
  $date = date_create($date_mask->format('Y-m-d'));
  date_sub($date, date_interval_create_from_date_string((empty($nbre_jours)?0:$nbre_jours)." days"));
  $date_sup = $date->format('d/m/Y');
  ...
  $sql .= " AND t.last_date::date >= '$date_sup'".(($etat == 2 || $etat == 3)?" AND a.etat_cpte = ".(($etat == 2)?8:4):"");
 ...
 *
 */
 function get_risques_credits($gestionnaire = 0, $export_date, $date_debloc_inf, $date_debloc_sup ,$id_prd, $zone) {
...
function get_liste_epargne($critere, $limit =NULL ,$offset=NULL, $date_rapport, $type_epargne=NULL, $zone = NULL) {
	global $dbHandler, $global_monnaie, $global_id_agence, $adsys;
	$db = $dbHandler->openConnection();
