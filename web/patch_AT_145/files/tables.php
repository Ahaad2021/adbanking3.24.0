...
$tables = array("ad_fer" => _("Jours feriés"),
		        ...
		        "adsys_zone_client" => _("Zone du client "),
    ...
    "adsys_niveau_agence" => _("Niveau Agence"),
		);
...
if ($global_nom_ecran == "Gta-1") {
  ...
  $js .= "else if (document.ADForm.HTML_GEN_LSB_table.value == 'adsys_zone_client') {assign('Lzc-1');}\n";
    $js .= "else if (document.ADForm.HTML_GEN_LSB_table.value == 'adsys_niveau_agence') {assign('Lob-1');}\n";
...
/*{{{ Cta-1 : Consultation d'une table */
  ...
  if ($SESSION_VARS['table'] == "adsys_produit_epargne") {
    ...
    $ordre = array_merge($ordre,array("infoscommissions","frais_tenue_cpt", "frequence_tenue_cpt","type_duree_min2retrait", "frais_retrait_cpt", "frais_depot_cpt", "frais_ouverture_cpt", "frais_fermeture_cpt", "frais_transfert", "frais_duree_min2retrait", "mode_calcul_frais_inactif", "mode_calcul_frais_dormant"));
    if ($PROD["classe_comptable"] != 2 and $PROD["classe_comptable"] != 5) {
...
#==========================================================================================================================
  ...
  $MyPage->addHTMLExtraCode("html_js",$html_jquery);
  ...
  case "adsys_produit_epargne":
    ...
    $MyPage->addHTMLExtraCode("infoscommissions", $tmp);
    $MyPage->setHTMLExtraCodeProperties("infoscommissions",HTMP_IN_TABLE, true);
    $ordre = array_merge($ordre, array("infoscommissions", "frais_tenue_cpt", "frequence_tenue_cpt", "frais_retrait_spec", "frais_retrait_cpt", "frais_depot_cpt", "frais_ouverture_cpt", "frais_fermeture_cpt", "frais_transfert", "type_duree_min2retrait", "frais_duree_min2retrait", "mode_calcul_frais_inactif", "mode_calcul_frais_dormant"));
    ...
    if (isMultiAgence()) {
      ...
      if (isAgencyBanking()){
        $ordre = array_merge($ordre, array("infoscommissionsod", "comm_depot_od", "comm_depot_od_mnt_min", "comm_depot_od_mnt_max", "comm_retrait_od", "comm_retrait_od_mnt_min", "comm_retrait_od_mnt_max","appl_comm_deplace"));
      }else {
    ...
    $ordre = array_merge($ordre,array("infosgen", "is_produit_actif", "nbre_occurrences","marge_tolerance","nbre_jours_report_debit", "nbre_jours_report_credit", "passage_etat_inactif","passage_etat_dormant"));
  ...
  if (isAgencyBanking()){;
    ...
    $ary_exclude = array_merge($ary_exclude, array("sens", "service_financier","retrait_unique","depot_unique","certif","mode_paiement","cpte_cpta_prod_ep_commission","mnt_commission","appl_comm_deplace"));
  }
  ...
  case "adsys_produit_credit":
    ...
    case "cpte_cpta_prod_cr_frais":
        if(isMobileLending()) {
            $info_prod = getProdInfo(" WHERE id = " . $_POST['contenu']);
            $flag_mob_lending = $info_prod[0]['is_mobile_lending_credit'];
            if ($flag_mob_lending == 't') {
                $info[$key]["choices"] = getNomsComptesComptables(array("compart_cpte IN(4, 1, 2) AND classe_compta IN (4, 7) ORDER BY id_ag, num_cpte_comptable ASC/*" => "*/--")); // Produit
            } else {
                $info[$key]["choices"] = getNomsComptesComptables(array("compart_cpte" => 4)); // Produit
            }
        }else{
  ...
  case "ad_agc":
    ...
    if (isMobileLending()){
      $ordre = array("infosadm", "libel_ag", "code_institution", "libel_institution","adresse", "tel", "fax","email","responsable", "type_structure", "date_agrement", "num_agrement", "num_tva","affil_reseau","type_numerotation_compte", "code_banque", "code_swift_banque", "code_ville", "code_antenne","delai_max_eav","nbre_part_social_max_cli"  ,"nbre_car_min_pwd","duree_pwd","imprim_coordonnee","has_cpte_cmplt_agc","identification_client","ml_localisation", "logo", "infosfin","quotite", "pp_montant_droits_adhesion", "pm_montant_droits_adhesion","gi_montant_droits_adhesion", "gs_montant_droits_adhesion", "tranche_frais_adhesion","capital_sociale_autorise","capital_sociale_souscrites","capital_sociale_lib" ,"val_nominale_part_sociale", "tranche_part_sociale", "prc_com_vir", "mnt_com_vir","constante_comm_change","mnt_retrait_ewallet_max_journalier","infosepargne","base_taux_epargne","alerte_dat_jours","param_affiche_billetage","param_affiche_solde","validite_chq_ord","validite_chq_cert","validite_ord_pay","applique_frais_tenue_cpte_gs","infoscredit", "tx_remb_anticipe","octroi_credit_non_soc","base_taux", "report_ferie", "duree_min_avant_octr_credit", "paiement_parts_soc_gs", "passage_perte_automatique", "dcr_lsb_detail_obj_credit", "infoscompta", "cloture_per_auto", "frequence_clot_per", "num_cpte_resultat","cpte_cpta_coffre", "plafond_retrait_guichet", "montant_plafond_retrait","plafond_retrait_deplace_guichet","montant_plafond_retrait_deplace", "cpte_tva_dec", "cpte_tva_rep","autorisation_approvisionnement_delestage","bloc_crea_cli");
    }else{
  ...
  case "adsys_produit_credit":
    ...
    $ordre = array_merge($ordre,array("infoscal","tx_interet","mode_calc_int","periodicite","freq_paiement_cap","typ_pen_pourc_dcr","prc_penalite_retard","mnt_penalite_jour","max_jours_compt_penalite","is_taux_mensuel"));
	    $ordre = array_merge($ordre,array("infosgen","type_duree_credit","duree_min_mois","duree_max_mois","nbre_reechelon_auth","differe_jours_max","differe_ech_max","differe_epargne_nantie","calcul_interet_differe","delai_grace_jour","nb_jr_bloq_cre_avant_ech_max","approbation_obli","gs_cat","prelev_frais_doss","report_arrondi","percep_frais_com_ass","ordre_remb","remb_cpt_gar","is_produit_actif", "is_flexible","is_produit_decouvert", "prelev_garanti", "appl_interet_diff_echeance", "is_mobile_lending_credit","crdt_salairie"));
    $ordre = array_merge($ordre,array("infosfin","mnt_min", "mnt_max","mnt_frais","prc_frais","prc_commission","mnt_commission")); 
    $ordre = array_merge($ordre,array("infosgar","prc_gar_num", "prc_gar_mat", "prc_gar_tot", "prc_gar_encours","prc_assurance","mnt_assurance"));
    $ordre = array_merge($ordre,array("infoscompta","cpte_cpta_prod_cr_int","cpte_cpta_prod_cr_pen","cpte_cpta_prod_cr_gar","cpte_cpta_att_deb"));
    ...
    if(!isMobileLending()){
        $js_mob_lending = "document.getElementsByName('HTML_GEN_BOL_is_mobile_lending_credit')[0].disabled = true;";
        $MyPage->addJS(JSP_FORM, "JS_mob", $js_mob_lending);
    }
  ...
  case "adsys_tarification":
      ...
      $codejs = "
                  ...
                  function populateTypeFrais()
                  {
                    ...
                    if (document.ADForm.HTML_GEN_LSB_code_abonnement.value == 'sms' || document.ADForm.HTML_GEN_LSB_code_abonnement.value == 'atm' || document.ADForm.HTML_GEN_LSB_code_abonnement.value == 'credit') {
                        ...
                        valueToPush['ATM_TSC'] => 'Frais transaction ATM',
    ...
    if (in_array($adsys_tarification_type_de_frais, array('SMS_REG','SMS_MTH','SMS_FRAIS','ESTAT_REG','ESTAT_MTH','ATM_REG','ATM-TSC', 'CRED_FRAIS', 'CRED_COMMISSION', 'CRED_ASSURANCE',
    ...
    case "adsys_taxes":
      ...
      case 'adsys_param_epargne':
          $js_check = "
            $('[name=cpte_dormant_nbre_jour]').change(function(){
              if(($(this).val() != '') && (parseInt($(this).val()) >= parseInt($('[name=cpte_inactive_nbre_jour]').val()))){
                  alert('Le nombre de jours sans mouvement pour les comptes inactifs doit être inférieur au nombre de jours sans mouvement pour les comptes dormants');
                  $(this).val('');
              }
            });
            
            $('[name=cpte_inactive_nbre_jour]').change(function(){
              if(($(this).val() != '') && (parseInt($(this).val()) <= parseInt($('[name=cpte_dormant_nbre_jour]').val()))){
                  alert('Le nombre de jours sans mouvement pour les comptes dormants doit être supérieur au nombre de jours sans mouvement pour les comptes inactifs');
                  $(this).val('');
              }
            });
          ...
          $ordre = array('ntable', 'cpte_dormant_nbre_jour', 'cpte_dormant_frais_tenue_cpte', 'cpte_inactive_nbre_jour', 'cpte_inactive_frais_tenue_cpte', 'active_compte_sur_transaction');
          $MyPage->setOrder(NULL, $ordre);
          $MyPage->addJS(JSP_FORM, "js_check_inactif_dormant", $js_check);
        break;
  }
  ...
  if (($SESSION_VARS['table'] == "parts_sociales") || ($SESSION_VARS['table'] == "epargne_nantie")) {
    ...
    array_push($ary_exclude, "classe_comptable");
      ...
      $ary_exclude = array_merge($ary_exclude, array("libel","sens","terme","service_financier",
          ...
          "decouvert_max", "decouvert_frais","decouvert_frais_dossier","decouvert_frais_dossier_prc", "cpte_cpta_prod_ep", "tx_interet_debiteur", "tx_interet", "ep_source_date_fin",
          "mode_calcul_int", "freq_calcul_int", "mode_paiement", "marge_tolerance", "modif_cptes_existants", "nbre_jours_report_debit", "decouvert_annul_auto", "decouvert_validite", "frais_chequier", "seuil_rem_dav",
          "masque_solde_epargne", "passage_etat_inactif", "passage_etat_dormant", "mnt_dpt_min", "tx_interet_max", "is_produit_actif",
...
//    }
   ...
   // $code_js .= "\n document.ADForm.HTML_GEN_BOL_passage_etat_dormant.checked = true; ";
    ...
    $MyPage->addHTMLExtraCode("infosgen", $tmp);
    $MyPage->setHTMLExtraCodeProperties("infosgen",HTMP_IN_TABLE, true);
    $ordre = array_merge($ordre,array("infosgen", "is_produit_actif", "nbre_occurrences","marge_tolerance","nbre_jours_report_debit", "nbre_jours_report_credit","duree_min_retrait_jour", "passage_etat_inactif","passage_etat_dormant"));
    ...
    $ordre = array_merge($ordre,array("ntable","libel","devise"));
    $ordre = array_merge($ordre,array("infoscal","tx_interet","mode_calc_int","mode_perc_int","periodicite","freq_paiement_cap","typ_pen_pourc_dcr","prc_penalite_retard","mnt_penalite_jour","max_jours_compt_penalite","is_taux_mensuel"));
    $ordre = array_merge($ordre,array("infosgen","type_duree_credit","duree_min_mois","duree_max_mois","nbre_reechelon_auth","differe_jours_max","differe_ech_max","differe_epargne_nantie","calcul_interet_differe","delai_grace_jour","nb_jr_bloq_cre_avant_ech_max","approbation_obli","gs_cat","prelev_frais_doss","report_arrondi","percep_frais_com_ass","ordre_remb","remb_cpt_gar","is_produit_actif", "is_flexible", "prelev_garanti", "appl_interet_diff_echeance"));
    $ordre = array_merge($ordre,array("infosfin","mnt_min", "mnt_max","mnt_frais","prc_frais","prc_commission","mnt_commission"));
    $ordre = array_merge($ordre,array("infosgar","prc_gar_num", "prc_gar_mat", "prc_gar_tot", "prc_gar_encours","prc_assurance","mnt_assurance"));
    $ordre = array_merge($ordre,array("infoscompta","cpte_cpta_prod_cr_int","cpte_cpta_prod_cr_pen","cpte_cpta_prod_cr_gar","cpte_cpta_att_deb"));
...
/*}}}*/
/*{{{ Lzc-1 : Ajout d'une zone du client */
else if ($global_nom_ecran == 'Lzc-1') {
    $myForm = new HTML_GEN2(_("Gestion des zones clients"));

    $zone_dataset = getZone();
    ksort($zone_dataset);

    $html = "<script src=\"/lib/misc/js/lib/jquery.min.js\" type=\"text/javascript\"></script>";
    $html .= "
      <TABLE id='zone_dataset' align='center' bgcolor='#FDF2A6' border='1' cellspacing='2' cellpadding='3' style='margin-bottom:40px;'>
        <tr bgcolor='#FDF2A6'>
            <th align='center'>Code</th>
            <th align='center'>Libellé</th>
        </tr>";

    if(!empty($zone_dataset)) {
        foreach ($zone_dataset as $key => $value) {
            $html .= "
        <tr id='$key' data-crud-transaction='read'>
            <td align='center'>$key</td>
            <td><input id='libelle' name='libelle' type='text' value='".$value["libelle_zone"]."'/></td>
        </tr>";
        }
    }else{
        $html .= "
        <tr id='1' data-crud-transaction='create'>
            <td align='center'>1</td>
            <td><input id='libelle' name='libelle' type='text'/></td>
        </tr>";
    }
    $html .= "</TABLE>";

    $myForm->addHiddenType("serialized_table");
    $js_form = "
      var existing_node_ref = $('#zone_dataset').find('tr:last-child');
      var node_dataset_length = $('#zone_dataset').find('input[name *= libelle]').length;
      var deletion_cursor = node_dataset_length - 1;
      var deletion_status = false;
      
      $('#zone_dataset').find('input[name *= libelle]').change(function(){
        var parent_ref = $(this).parent().parent();
        if(parent_ref.attr('data-crud-transaction') != 'create'){
          parent_ref.attr('data-crud-transaction', 'update');
        }
      });
      
      function addInput(){
        var clone_node_ref = $(existing_node_ref).clone();
        var clone_node_id = getLastID();
        
        $(clone_node_ref).attr('data-crud-transaction', 'create');
        $(clone_node_ref).removeAttr('style');
        $(clone_node_ref).attr('id',clone_node_id);
        $(clone_node_ref).find('td:first').text(clone_node_id);
        $(clone_node_ref).find('td:last input').val('');
        $('#zone_dataset').append(clone_node_ref);
      }
      
      function deleteElement(){
        var node_ref = $('#zone_dataset').find('tr:last-child');
        if($(node_ref).attr('data-crud-transaction') == 'create'){
          $(node_ref).remove();
        }else{
          var node_dataset = $('#zone_dataset').find('[data-crud-transaction]');
          while(deletion_cursor >= 0){
            if($(node_dataset[deletion_cursor]).attr('data-crud-transaction') != 'delete'){
               $(node_dataset[deletion_cursor]).hide();
               $(node_dataset[deletion_cursor]).attr('data-crud-transaction', 'delete');
               break;
            }
            deletion_cursor--;
          }
      ...
      function getLastID(){
        var ref = $('#zone_dataset').find('tr[data-crud-transaction != delete]');
        if((ref.length - 1) == 0)
          return 1;
        return parseInt(ref.last().attr('id')) + 1;
      }
      
      function serializedTable(){
        var serialized_dataset = {table: []};
        var node_dataset = $('#zone_dataset').find('[data-crud-transaction]');
        
        $('#zone_dataset').find('[data-crud-transaction]').each(function(index){
          serialized_dataset.table.push({code: $(this).attr('id'), libelle: $(this).find('td:last input').val(), crud_transaction: $(this).attr('data-crud-transaction')});
        });
        
        $('input[name=serialized_table]').val(JSON.stringify(serialized_dataset));
      }
    ...
    $myForm->addJS(JSP_FORM, "misc_form_function", $js_form);
    $myForm->addHTMLExtraCode("html", $html);

    $myForm->addFormButton(1, 1, "butVal", _("Valider"), TYPB_BUTTON);
    $myForm->setFormButtonProperties("butVal", BUTP_JS_EVENT, array("onclick" => "if(isInputEmpty('#zone_dataset tr[data-crud-transaction] input[name=libelle]')){alert('Les palier doivent être tous renseigné'); ADFormValid=true;}else{serializedTable(); assign('Lzc-2'); this.form.submit();}"));
//    $myForm->setFormButtonProperties("butVal", BUTP_CHECK_FORM, false);
//    $myForm->setFormButtonProperties("butVal", BUTP_AXS, 754);
//    $myForm->setFormButtonProperties("butVal", BUTP_PROCHAIN_ECRAN, "Lzc-2");

    //Bouton ajout
    $myForm->addFormButton(1, 2, "butAjout", _("Ajouter"), TYPB_BUTTON);
    $myForm->setFormButtonProperties("butAjout", BUTP_JS_EVENT, array("onclick" => "addInput();"));

    //Bouton supprimé
    $myForm->addFormButton(1, 3, "butSupprimer", _("Supprimer"), TYPB_BUTTON);
    $myForm->setFormButtonProperties("butSupprimer", BUTP_JS_EVENT, array("onclick" => "deleteElement();"));
    $myForm->addHiddenType("nb_ligne", $SESSION_VARS['nb_ligne']);

    //Bouton retour
    $myForm->addFormButton(1, 4, "butRet", _("Retour"), TYPB_SUBMIT);
    $myForm->setFormButtonProperties("butRet", BUTP_CHECK_FORM, false);
    $myForm->setFormButtonProperties("butRet", BUTP_PROCHAIN_ECRAN, "Gta-1");

    $myForm->buildHTML();
    echo $myForm->getHTML();
}
//Supression, Modification est Ajout d'un zone
else if ($global_nom_ecran == 'Lzc-2'){
    $deserialized_dataset = json_decode($_POST["serialized_table"], true);

    $db = $dbHandler->openConnection();
    foreach($deserialized_dataset["table"] as $value){
      switch($value["crud_transaction"]){
          case 'read': break;
          case 'update':
              $DATA["libelle_zone"] = $value["libelle"];
              $result = executeQuery($db, buildUpdateQuery("adsys_zone_client", $DATA, array("code_zone" => $value["code"])));
              if (DB::isError($result)) {
                  $dbHandler->closeConnection(false);
            ...
            break;
          case 'delete':
              $DATA["code_zone"] = $value["code"];
              $result = executeQuery($db, "DELETE FROM adsys_zone_client WHERE code_zone = '".$DATA["code_zone"]."'");
              if (DB::isError($result)) {
                  $dbHandler->closeConnection(false);
                  signalErreur(__FILE__, __LINE__, __FUNCTION__, $result->getMessage());
              }
          ...
          case 'create':
              ...
              $result = executeQuery($db, buildInsertQuery("adsys_zone_client", $DATA));
              if (DB::isError($result)) {
                  $dbHandler->closeConnection(false);
                  signalErreur(__FILE__, __LINE__, __FUNCTION__, $result->getMessage());
              }
            break;
          default:
              signalErreur(__FILE__,__LINE__,__FUNCTION__, 'Invalid database operation');
      }
    }

    $db = $dbHandler->closeConnection(true);
    $zone_libelle = 'Gikaruka';
    $MyPage = new HTML_message(_("Confirmation d'un zone"));
    $MyPage->setMessage(sprintf(_("La zone a été bien modifiée"),$zone_libelle));
...
// ecran de valisation des donnees pour les localisations Rwanda
  ...
  if (isset($butval) && $butval == "Valider") {
...
else if ($global_nom_ecran == 'Lob-1') {
    $MyPage = new HTML_GEN2(_("Gestion de la table de paramétrage") . " Niveau Agence FENACOBU");
    //Si table générique
    $ary1 = array('adsys_niveau_agence');
    if(isset($SESSION_VARS['table'])){
        $table = $SESSION_VARS['table'];
    }
    if ((isset($table) && in_array($table, $ary1)) || in_array($SESSION_VARS['ajout_table'], $ary1) || in_array($SESSION_VARS['consult_table'], $ary1) || in_array($SESSION_VARS['modif_table'], $ary1)) {
    ...
    $MyPage->setFieldProperties("contenu", FIELDP_HAS_CHOICE_AUCUN, true);
    $MyPage->setFieldProperties("contenu", FIELDP_IS_REQUIRED, true);
    $liste_localisationFENACOBU = getListelocalisationFenacobu();
    //Trier par ordre alphabétique
    natcasesort($liste_localisationFENACOBU);
    if (sizeof($liste_localisationFENACOBU) > 0)
        $MyPage->setFieldProperties("contenu", FIELDP_ADD_CHOICES, $liste_localisationFENACOBU);

    //$MyPage->addButton("contenu", "butcons", _("Consulter"), TYPB_SUBMIT);
    //$MyPage->setButtonProperties("butcons", BUTP_PROCHAIN_ECRAN, "Gcf-1");
    //$MyPage->setButtonProperties("butcons", BUTP_AXS, 252);
    $MyPage->addButton("contenu", "butmodif", _("Modifier"), TYPB_SUBMIT);
    $MyPage->setButtonProperties("butmodif", BUTP_PROCHAIN_ECRAN, "Lob-2");

    //Bouton formulaire
    $MyPage->addFormButton(1, 1, "butajou", _("Ajouter"), TYPB_SUBMIT);
    $MyPage->setFormButtonProperties("butajou", BUTP_CHECK_FORM, false);
    $MyPage->setFormButtonProperties("butajou", BUTP_PROCHAIN_ECRAN, "Lob-2");
    //$MyPage->setFormButtonProperties("butajou", BUTP_AXS, 252);

    $MyPage->addFormButton(1, 2, "butsuppr", _("Supprimer"), TYPB_SUBMIT);
    $MyPage->setFormButtonProperties("butsuppr", BUTP_CHECK_FORM, false);
    $MyPage->setFormButtonProperties("butsuppr", BUTP_PROCHAIN_ECRAN, "Lob-3");
...
else if ($global_nom_ecran == 'Lob-2') {
    if (isset($butajou) && $butajou = 'Ajouter') {
        $MyPage = new HTML_GEN2(_("Ajout nouvelle localisation FENACOBU"));
        $array_table = array('adsys_niveau_agence'=>_("Niveau Agence"));
        //Nom table
        $MyPage->addField("ntable", _("Table de paramétrage"), TYPC_TXT);
        $MyPage->setFieldProperties("ntable", FIELDP_IS_LABEL, true);
        $MyPage->setFieldProperties("ntable", FIELDP_DEFAULT, $array_table[$SESSION_VARS['nom_table']]);
        ...
        $codejs = " function populateParent()
      {
        if (document.ADForm.HTML_GEN_LSB_type_niveau.value > 1) {
            var _cQueue = [];
            var valueToPush = {};
            if (document.ADForm.HTML_GEN_LSB_type_niveau.value == 2){
              ";
        $where = "type_niveau = 1";
        $valueToPush = getListelocalisationFENACOBU($where);
        while (list($key, $value) = each($valueToPush)) {
            $codejs .= " valueToPush['" . $key . "'] = '" . $value . "';";
        }
        $codejs .= "} if (document.ADForm.HTML_GEN_LSB_type_niveau.value == 3){
              ";
        $where = "type_niveau = 2";
        $valueToPush = getListelocalisationFENACOBU($where);
        while (list($key, $value) = each($valueToPush)) {
...
//        }
        ...
        $MyPage->addJS(JSP_FORM, "JS1", $codejs);
        $MyPage->setFieldProperties("libelle_niveau", FIELDP_IS_REQUIRED, true);
        $MyPage->setFieldProperties('type_niveau', FIELDP_TYPE, TYPC_LSB);
        $MyPage->setFieldProperties("type_niveau", FIELDP_HAS_CHOICE_AUCUN, true);
        $MyPage->setFieldProperties('type_niveau', FIELDP_ADD_CHOICES, $adsys["type_niveau_agence"]);
        $MyPage->setFieldProperties("type_niveau", FIELDP_JS_EVENT, array("onChange" => "populateParent();"));
        $MyPage->setFieldProperties("type_niveau", FIELDP_IS_REQUIRED, true);
        $MyPage->setFieldProperties('parent', FIELDP_TYPE, TYPC_LSB);
        $MyPage->setFieldProperties("parent", FIELDP_HAS_CHOICE_AUCUN, true);
        ...
        //Bouton
        $MyPage->addFormButton(1, 1, "butval", _("Valider"), TYPB_SUBMIT);
        $MyPage->setFormButtonProperties("butval", BUTP_PROCHAIN_ECRAN, "Lob-3");
        //$MyPage->setFormButtonProperties("butval", BUTP_JS_EVENT, array("onclick" => $checkDateSaison));
        $MyPage->addFormButton(1, 2, "butret", _("Annuler"), TYPB_SUBMIT);
        $MyPage->setFormButtonProperties("butret", BUTP_CHECK_FORM, false);
        $MyPage->setFormButtonProperties("butret", BUTP_PROCHAIN_ECRAN, "Lob-1");
        ...
        $MyPage->addField("ntable", _("Table de paramétrage"), TYPC_TXT);
        $MyPage->setFieldProperties("ntable", FIELDP_IS_LABEL, true);
        $MyPage->setFieldProperties("ntable", FIELDP_DEFAULT, $array_table[$SESSION_VARS['modif_table']]);
        ...
        foreach ($info as $key => $value) {
            //Pour chaque champs de la table
            if (($key != "pkey") && (!in_array($key, $array_table))) //On n'insère pas les clés primaires
            {  //On n'insère pas certains champs en fonction du contexte
                if (!$value['ref_field']) {  //Si champs ordinaire
                    $type = $value['type'];
                    if ($value['traduit'])
            ...
            if ($SESSION_VARS['modif_table'] == 'adsys_niveau_agence') {
                if ($key == 'parent') {
                    $where = "id = " . $value['val'];
                    $valueParent = getListelocalisationFENACOBU($where);
                    $MyPage->setFieldProperties($key, FIELDP_DEFAULT, $value['val']);
                }
            }
        }
            ...
            $codejs = " function populateParent()
      {
        if (document.ADForm.HTML_GEN_LSB_type_niveau.value > 1) {
            ...
            if (document.ADForm.HTML_GEN_LSB_type_niveau.value == 2){
              ";
            $where1 = "type_niveau = 1";
            $valueToPush = getListelocalisationFENACOBU($where1);
            while (list($key, $value) = each($valueToPush)) {
                $codejs.=" valueToPush['".$key."'] = '".$value."';";
            }
            $codejs.="} if (document.ADForm.HTML_GEN_LSB_type_niveau.value == 3){
              ";
            $where2 = "type_niveau = 2";
            $valueToPush = getListelocalisationFENACOBU($where2);
            while (list($key, $value) = each($valueToPush)) {
                $codejs.=" valueToPush['".$key."'] = '".$value."';";
            }
//            $codejs.="} if (document.ADForm.HTML_GEN_LSB_type_localisation.value == 4){
...
//            $MyPage->setFieldProperties('type_niveau', FIELDP_TYPE, TYPC_LSB);
//            $MyPage->setFieldProperties("type_niveau", FIELDP_HAS_CHOICE_AUCUN,true);
//            $MyPage->setFieldProperties('type_niveau', FIELDP_ADD_CHOICES, $adsys["type_niveau_fenacobu"]);
            $MyPage->setFieldProperties("type_niveau", FIELDP_JS_EVENT, array("onChange" => "populateParent();"));

            $where = "id = ".($contenu);
            $valueDetails = getLocalisationFENACOBUDetails($where);;
            if ($valueDetails[$contenu]['type_niveau'] == 2 || $valueDetails[$contenu]['type_niveau']==3){
                $new_type_niveau = $valueDetails[$contenu]['type_niveau'] - 1;
                $valueParentList = getListelocalisationFENACOBU (" type_niveau = ".$new_type_niveau);
                $MyPage->setFieldProperties('parent', FIELDP_TYPE, TYPC_LSB);
                $MyPage->setFieldProperties('parent', FIELDP_HAS_CHOICE_AUCUN,true);
                $MyPage->setFieldProperties('parent', FIELDP_ADD_CHOICES, $valueParentList);
            }else{
        ...
        //Bouton
        $MyPage->addFormButton(1, 1, "butvalmodif", _("Valider"), TYPB_SUBMIT);
        $MyPage->setFormButtonProperties("butvalmodif", BUTP_PROCHAIN_ECRAN, "Lob-3");
        //$MyPage->setFormButtonProperties("butval", BUTP_JS_EVENT, array("onclick" => $checkDateSaison));
        $MyPage->addFormButton(1, 2, "butret", _("Annuler"), TYPB_SUBMIT);
        $MyPage->setFormButtonProperties("butret", BUTP_CHECK_FORM, false);
        $MyPage->setFormButtonProperties("butret", BUTP_PROCHAIN_ECRAN, "Lob-1");
...
else if ($global_nom_ecran == 'Lob-3') {
    global $global_id_agence, $global_monnaie;
    $ary_exclude = array();
    ...
    if (isset($butval) && $butval == "Valider") {
        ...
        if ($myErr->errCode == NO_ERR) {
            ...
            $message = sprintf(_("L'entrée été ajoutée avec succès"));
            $MyPage->setMessage($message);
            $MyPage->addButton(BUTTON_OK, "Lob-1");
            $MyPage->buildHTML();
            echo $MyPage->HTML_code;
        } else {
            $MyPage = new HTML_erreur(_("Echec de l'insertion"));
    ...
    else if (isset($butsuppr) && $butsuppr == "Supprimer") {

        $boolean_check = true;
        //check niveau Agence/Guichet
        $conditionCheckNiveau = "id = $contenu";
        $check_niveau =  getLocalisationFENACOBUDetails($conditionCheckNiveau);


        //verifier si l'agence ou guichet possede des child
        $condiLocWhere = "parent = $contenu";
        $check_child =  getLocalisationFENACOBUDetails($condiLocWhere);

        if ($check_niveau[$contenu]["type_localisation"] == 1){
            $conditionCliWhere = " coopec = ".$check_niveau[$contenu]["id"];
            $data_cli = getClientFENACOBUDetails($conditionCliWhere);
            if (($check_child != null) || ($data_cli != null)){
                $boolean_check = false;
            }
        }else if ($check_niveau[$contenu]["type_localisation"] == 2){
            $conditionCliWhere = " agence = ".$check_niveau[$contenu]["id"];
            $data_cli = getClientFENACOBUDetails($conditionCliWhere);
            if (($check_child != null) || ($data_cli != null)){
                $boolean_check = false;
            }
        }else if ($check_niveau[$contenu]["type_localisation"] == 3){
            $conditionCliWhere = " guichet = ".$check_niveau[$contenu]["id"];
            $data_cli = getClientFENACOBUDetails($conditionCliWhere);
            if (($check_child != null) || ($data_cli != null)){
                $boolean_check = false;
            }
        ...
        if ($boolean_check == true){
            $del_data = deleteData("adsys_localisation_fenacobu", array("id" => $check_niveau[$contenu]["id"]));
            if ($myErr->errCode == NO_ERR) {
                $MyPage = new HTML_message(_("Confirmation supression"));
                $MyPage->setMessage(sprintf(_("Votre donnée a été supprimée!")));
                $MyPage->addButton(BUTTON_OK, "Lob-1");
                $MyPage->buildHTML();
                echo $MyPage->HTML_code;
            }
        }else {
            $erreur = new HTML_erreur(_("Echec de la supression"));
            $erreur->setMessage(_("Votre donnée possède des dependances. Veuillez accomoder votre dependance avant la supression"));
            $erreur->addButton(BUTTON_OK,"Lob-1");
            $erreur->buildHTML();
