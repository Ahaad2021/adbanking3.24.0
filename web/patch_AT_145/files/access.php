        ...
        if(!empty($liste_gars)) { // check for null
            foreach($liste_gars as $cle=>$valeur) {
                if ($valeur['etat_gar'] == 3) { //garantie mobilisée
                    return true;
                } elseif($value['etat'] == 5 AND $value['gar_num_encours']>0 AND $value['cpt_gar_encours']==$valeur['gar_num_id_cpte_nantie']) {
...
function check_fonction_136() {
    ...
    if(check_fonction_110() || check_fonction_137() || check_fonction_138() ||  check_fonction_139() || check_fonction_141() || check_fonction_142() || check_fonction_143() || check_fonction_144() || check_fonction_145() || check_fonction_104()) {
        $return_val = true;
    }
...
function check_fonction_141() {
  ...
  // Informations sur le client
  $infos_client = getClientDatas($global_id_client);
  $cre_etat_all = implode(', ', array_keys(getTousEtatCreditAll()));

  // On prend tous les dossiers individuels déboursés du client
  // [5] - "Fonds déboursés"
  $whereCl =" AND is_ligne_credit='f' AND (etat=5) AND (cre_etat IN (".$cre_etat_all.")) AND (gs_cat != 2 OR gs_cat IS NULL)";
...
function check_fonction_145() {
  ...
  // On prend tous les dossiers individuels déboursés du client
  $cre_etat_all = implode(', ', array_keys(getTousEtatCreditAll()));
  $whereCl =" AND is_ligne_credit='f' AND etat=5 AND (cre_etat IN (".$cre_etat_all.")) AND periodicite != 6 AND (gs_cat != 2 OR gs_cat IS NULL)";
  $elt_dossier = getIdDossier($global_id_client, $whereCl);
...
function check_fonction_91() { // Compte dormant 
  global $global_id_client;
  $ListeComptes = getComptesDormantsInactifs($global_id_client);
...
function check_fonction_765() { // Gestion des annulations retrait et dépôt via agent
  global $global_id_client, $global_nom_login;

  if (
    hasOperationEpargneViaAgent($global_nom_login)
    || hasDemandeAnnulationEnregistreViaAgent()
    || hasDemandeAnnulationAutoriseViaAgent($global_nom_login)
  )
  {
    return true;
  }
...
function check_fonction_766() { // Demande annulation retrait / dépôt Via agent
  global $global_id_client, $global_nom_login;

  if (hasOperationEpargneViaAgent($global_nom_login)) {
    return true;
  }
...
function check_fonction_767() { // Approbation demande annulation retrait / dépôt Via agent
  global $global_id_client,$global_nom_login;

  if (hasDemandeAnnulationEnregistreViaAgent($global_nom_login)) {
    return true;
  }
...
function check_fonction_768() { // Effectuer annulation retrait / dépôt Via Agent
  global $global_id_client,$global_nom_login;

  if (hasDemandeAnnulationAutoriseViaAgent($global_nom_login)) {
    return true;
  }
...
function check_fonction_757() {

  if (sizeof(getListeApprovisionnementTransfert(null,1)) > 0){
    return true;
  }
  else{
    return false;
...
function check_fonction_758() {

  if (sizeof(getListeApprovisionnementTransfert(null,2)) > 0){
    return true;
  }
  else{
    return false;
...
function check_fonction_752() {

  if (sizeof(getProfilAgent()) > 0){
    return true;
  }
  else{
    return false;
...
function check_fonction_782() {

  if (sizeof(getClientDatasAgent(10)) > 0){
    return true;
  }
  else{
    return false;
...
function check_fonction_783() {
global $global_id_utilisateur;
  $client_data = getClientDatasAgent(11,null,$global_id_utilisateur);
...
function check_fonction_104() {
    ...
    // Dossiers individuels du client
    $whereCl=" AND is_ligne_credit='f' AND (etat = 16) AND (gs_cat != 2 OR gs_cat IS NULL)";
    $elt_dossier= getIdDossier($global_id_client,$whereCl);

    $nbre_elt = count($elt_dossier);// récupère le nombre de dossiers du client

    $nbre_dcr_date_remb = 0;
    if(count(Historisation::getListDossierHis(null, 4, 'f')) > 0) {
        $nbre_dcr_date_remb++;
    }
    ...
    if ($infos_client['statut_juridique'] == 4) {
        $dossiers_membre = getDossiersMultiplesGS($global_id_client);
        foreach($dossiers_membre as $id_doss=>$value)
            if ($value['is_ligne_credit'] == 'f' && $value['etat'] == 16)
                $nbre_elt++;
    }
...
function check_fonction_13() {
    global $global_id_client;
    $infos_client = getClientDatas($global_id_client);
    if ($infos_client['statut_juridique']==1){
        return true;
    }else{
        return false;
    }
}
function check_fonction_47() {
    global $global_id_client;
    $data_carte = getRetraitCarte($global_id_client);
    if (sizeof($data_carte) > 0){
        return true;
    }else{
        return false;
    }
