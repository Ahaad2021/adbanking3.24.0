...
if ($global_nom_ecran == "Mcl-1") {
  ...
  $AG = getAgenceDatas($global_id_agence);
    ...
    if($AG["is_zone_required"] == 't'){
        update_d_tableliste(array("isreq" => "t"), array("nchmpc" => "client_zone"));
    }else{
        update_d_tableliste(array("isreq" => "f"), array("nchmpc" => "client_zone"));
    }
  ...
  if ($AGC['identification_client']==2){
    $exclude = array("raison_defection", "gi_date_dissol", "nbre_parts", "raison_defection", "date_defection", "classe","id_loc1","id_loc2","loc3","district","secteur","cellule","village");
      ...
      if (isFusion()){
          ...
          $exclude = array("raison_defection", "gi_date_dissol", "nbre_parts", "raison_defection", "date_defection", "classe","id_loc2","province","district","secteur","cellule","village","classe_socio_economique","education","n_agence","n_guichet");
      }
  ...
  if ($SESSION_VARS["statut_juridique"] == 1) {     // Personne physique
    ...
    if ($AGC['identification_client'] == 2){
      array_push($Order, "gi_nom", "adresse", "code_postal", "ville", "pays", "num_tel", "num_fax", "num_port", "email", "province","district","secteur","cellule","village", "client_zone", "sect_act", "langue", "gi_nbre_membr", "nbre_hommes_grp", "nbre_femmes_grp", "nb_imf", "nb_bk", "gi_date_agre", "etat", "qualite", "commentaires_cli");
    }else{
      array_push($Order, "gi_nom", "adresse", "code_postal", "ville", "pays", "num_tel", "num_fax", "num_port", "email", "id_loc1", "id_loc2", "loc3", "client_zone",  "sect_act", "langue", "gi_nbre_membr", "nbre_hommes_grp", "nbre_femmes_grp", "nb_imf", "nb_bk", "gi_date_agre", "etat", "qualite", "commentaires_cli");
    }
    ...
    if ($AGC['identification_client'] == 2){
      array_push($Order, "gi_nom", "gs_responsable","adresse", "code_postal", "ville", "pays", "num_tel", "num_fax", "num_port", "email", "province","district","secteur","cellule","village", "client_zone", "sect_act", "langue", "nb_imf", "nb_bk", "etat", "qualite", "commentaires_cli");
    }else{
      array_push($Order, "gi_nom", "gs_responsable","adresse", "code_postal", "ville", "pays", "num_tel", "num_fax", "num_port", "email", "id_loc1", "id_loc2", "loc3", "client_zone",  "sect_act", "langue", "nb_imf", "nb_bk", "etat", "qualite", "commentaires_cli");
    }
  }
  ...
  $myForm->setFieldProperties("categorie", FIELDP_JS_EVENT, array("onchange" => "displayCat()"));
    ...
    // --> Restriction des choix dans localisation fenacobu
    $myForm->setFieldProperties("n_agence", FIELDP_INCLUDE_CHOICES, $includeChoicesFenacobu);
    // --> Construction de la fonction de mise à jour du district
    $jsCodeNivFenacobu = "function displayNivGuichet() {\n";
    $jsCodeNivFenacobu .= "console.log('test');";
    ...
    while (list(, $value) = each($nivArrayFenacobu)) {
        if ($value['parent'] != 0) {
            $jsCodeNivFenacobu .= "\tif (document.ADForm.HTML_GEN_LSB_n_agence.value == " . $value['parent'] . ")\n";
            $jsCodeNivFenacobu .= "\t\tdocument.ADForm.HTML_GEN_LSB_n_guichet.options[document.ADForm.HTML_GEN_LSB_n_guichet.length] = new Option('" . $value['libelle_niveau'] . "', '" . $value['id'] . "', false, false);\n";
        }
    }
    $jsCodeNivFenacobu .= "\n}";
    // --> Constrution des choix disponibles pour le district sélectionné
    ...
    reset($nivArrayFenacobu);
    $parent = $CLI['n_agence'];
    if (isset($n_agence)) { //AT-150 page reloaded take value from posted data
        $parent = $n_agence;
    }
    while (list(, $value) = each($nivArrayFenacobu)) {
        if ($value['parent'] == $parent)
            //$choices[$value['id']] = $value['libelle_localisation'];
            array_push($choices, $value['id']);
    }
    // --> Ajout de la fonction dans le formulaire
    $myForm->addJS(JSP_FORM, "displayNivGuichet", $jsCodeNivFenacobu);
    // --> ajout des champs
    //  $myForm->addField("n_guichet", _("Niveau Guichet"), TYPC_LSB);
    $myForm->setFieldProperties("n_guichet", FIELDP_DEFAULT, $CLI['n_guichet']);
    $myForm->setFieldProperties("n_guichet", FIELDP_INCLUDE_CHOICES, $choices);
    $myForm->setFieldProperties("n_guichet", FIELDP_IS_REQUIRED, true);
    $myForm->setFieldProperties("n_agence", FIELDP_JS_EVENT, array("onchange" => "displayNivGuichet()"));

    array_push($Order, "n_agence");
    array_push($Order, "n_guichet");
    }
...
/*{{{ Mcl-2 : Confirmation de la mise à jour du client */
  ...
  $temp = getClientDatas($global_id_client);
  ...
  if ($myErr->errCode != NO_ERR) {
    ...
    $temp_global = array(
        "photo" => $global_photo_client,
        "signature" => $global_signature_client,
    );
    ...
    if(empty($temp_global["photo"])){
        $temp_global["photo"] = '/adbanking/images/travaux.gif';
        $temp_global["signature"] = '/adbanking/images/travaux.gif';
    }

    $erreur = audit_client($temp);
    ajout_historique (10, $global_id_client, $erreur->param, $global_nom_login, date("r"), NULL);
