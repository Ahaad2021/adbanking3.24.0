...
$fonc_sys_delimiter = array(array('counter'=>251, 'limit'=>251),array('counter'=>260, 'limit'=>260),array('counter'=>751, 'limit'=>799));
...
/*{{{ Gpa-1 : Gestion des profils */
if ($global_nom_ecran == "Gpa-1") {
    ...
    //Champs profil
    $profils = getProfilAgent();
    $MyPage->addField("cprofil",_("Profil"),TYPC_LSB);
    $MyPage->setFieldProperties("cprofil", FIELDP_IS_REQUIRED, true);
    $MyPage->setFieldProperties("cprofil", FIELDP_HAS_CHOICE_AUCUN, true);
    $MyPage->setFieldProperties("cprofil",FIELDP_ADD_CHOICES, $profils);
    ...
    $MyPage->setButtonProperties("consult", BUTP_AXS, 779);
    $MyPage->setButtonProperties("consult", BUTP_PROCHAIN_ECRAN, "Cpr-1");

    //Bouton modifier
    $MyPage->addButton("cprofil", "modif", _("Modifier"), TYPB_SUBMIT);
    $MyPage->setButtonProperties("modif", BUTP_AXS, 778);
    $MyPage->setButtonProperties("modif", BUTP_PROCHAIN_ECRAN, "Mpr-1");

    //Bouton supprimer
    $MyPage->addButton("cprofil", "supr", _("Supprimer"), TYPB_SUBMIT);
    $MyPage->setButtonProperties("supr", BUTP_AXS, 780);
    $MyPage->setButtonProperties("supr", BUTP_PROCHAIN_ECRAN, "Spr-1");

    //Bouton créer
    $MyPage->addFormButton(1, 1, "cree", _("Créer un nouveau profil"), TYPB_SUBMIT);
    $MyPage->setFormButtonProperties("cree", BUTP_AXS, 777);
    $MyPage->setFormButtonProperties("cree", BUTP_PROCHAIN_ECRAN, "Apa-1");
...
/*{{{ Mpa-1 : Modification d'un profil */
else if ($global_nom_ecran == "Mpr-1") {
    if ($cprofil == 1) {
    ...
    foreach ($fonc_sys_delimiter as $val) {
        ...
        for ($i=$val['counter']; $i<=$val['limit']; ++$i) { //Pour toutes les fonctions
            switch ($i) { //Affiche les sous-titres
                ...
                case 751 :
                    if(isAgencyBanking()) {
                        $MyPage->addHTMLExtraCode("html$i", "<b>" . _("Module Agency Banking") . "</b>");
                        $MyPage->setHTMLExtraCodeProperties("html$i", HTMP_IN_TABLE, true);
                    }
                    break;
                case 800 :
            ...
            if (isset($adsys["adsys_fonction_systeme"][$i])) { // Si la $i ème fonction est définie
                ...
                // s'il le profil est pour un agent
                if (isAgencyBanking()) {
                    if (isset($adsys["adsys_exclu_fonc_gestion"]) && (is_array($adsys["adsys_exclu_fonc_gestion"]))
                    ) {
                        // Si le profil nécessite un guichet, désactiver la fonction $i si elle est exclusive à la présence de guichet
                        if (($global_nom_ecran == "Mpr-1") && (in_array($i, $adsys["adsys_exclu_fonc_gestion"]))) {
                            $MyPage->setFieldProperties("fonction$i", FIELDP_DEFAULT, false);
                            $MyPage->setFieldProperties("fonction$i", FIELDP_IS_LABEL, true);
                        }
                    }
    ...
    $MyPage->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, "Mpr-2");
    $MyPage->setFormButtonProperties("ok", BUTP_KEY, KEYB_ENTER);

    //Bouton annuler
    $MyPage->addFormButton(1, 2, "annul", _("Annuler"), TYPB_SUBMIT);
    $MyPage->setFormButtonProperties("annul", BUTP_PROCHAIN_ECRAN, "Gpa-1");
...
/*{{{ Mpa-2 : Modification d'un profil (confirmation) */
else if ($global_nom_ecran == "Mpr-2") {
    ...
    $Err = update_profil_agent($SESSION_VARS['cprofil'], $libel, $timeout, isset($conn_agc), isset($masque_solde), $fonctions, isset($masque_solde_vip));
    ...
    $MyPage->addButton(BUTTON_OK, "Gpa-1");
...
/*{{{ Spr-1 : Suppression d'un profil (demande de confirmation) */
else if ($global_nom_ecran == "Spr-1") {
    if ($cprofil == 1) {
    ...
    if (sizeof($SESSION_VARS['logins']) > 0) { //Si il y a encore des logins attachés : erreur
        ...
        $MyPage->addButton(BUTTON_OUI, "Spr-2");
        $MyPage->addButton(BUTTON_NON, "Gpa-1");
...
else if ($global_nom_ecran == "Spr-2") { //Suppression effective d'un profil, confirmation
...
/*{{{ Apa-1 : Ajout d'un profil */
else if ($global_nom_ecran == "Apa-1") {
    $javascript = "";

    //Début génération HTML
    $MyPage = new HTML_GEN2(_("Création profil pour agent"));
    ...
    $MyPage->addField("guichet", _("Le profil possède-t-il un guichet ?"), TYPC_BOL);
    $MyPage->setFieldProperties("guichet", FIELDP_JS_EVENT, array("onclick"=>"return false;"));
    //$MyPage->setFieldProperties("guichet", FIELDP_JS_EVENT, array("onchange"=>"verif_guichet();"));
    //$javascript .= get_guichet_fct();
    $javascript .= "document.ADForm.HTML_GEN_BOL_guichet.checked = true;";
    ...
    foreach ($fonc_sys_delimiter as $val) {
        ...
        for ($i=$val['counter']; $i<=$val['limit']; ++$i) { //Pour toutes les fonctions
            ...
            if (isset($adsys["adsys_fonction_systeme"][$i])) { //Si la $i ème fonction est définie
                ...
                // s'il le profil est pour un gestionnaire
                if (isAgencyBanking()) {
                    if (isset($adsys["adsys_exclu_fonc_gestion"]) && (is_array($adsys["adsys_exclu_fonc_gestion"]))
                    ) {
                        // Si le profil nécessite un guichet, désactiver la fonction $i si elle est exclusive à la présence de guichet
                        if (($global_nom_ecran == "Apa-1") && (in_array($i, $adsys["adsys_exclu_fonc_gestion"]))) {
                            $MyPage->setFieldProperties("fonction$i", FIELDP_DEFAULT, false);
                            $MyPage->setFieldProperties("fonction$i", FIELDP_IS_LABEL, true);
                        }
                    }
    ...
    //Bouton valider
    $MyPage->addFormButton(1, 1, "ok", _("Valider"), TYPB_SUBMIT);
    $MyPage->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, "Apa-2");
...
/*{{{ Apa-2 : Ajout d'un profil (confirmation) */
else if ($global_nom_ecran == "Apa-2") {
    ...
    if (! in_array(0, $fonctions)) $fonctions[$i] = 0; //Tout le monde a accès à la fonction 0
    //Insère profil
    cree_profil_agent($libel, $fonctions, isset($guichet), $timeout, isset($masque_solde), isset($masque_solde_vip));
...
/*{{{ Cpr-1 : Consultation d'un profil */
else if ($global_nom_ecran == "Cpr-1") {
    $SESSION_VARS['profil_axs'] = get_profil_axs($cprofil); //Recherche l'info du profil choisit
    $SESSION_VARS['nom_profil'] = get_profil_nom($cprofil); //Recherche le libellé du profil
    $SESSION_VARS['cprofil'] = $cprofil;
    ...
    $MyPage->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, "Gpa-1");
    $MyPage->setFormButtonProperties("ok", BUTP_KEY, KEYB_ENTER);
