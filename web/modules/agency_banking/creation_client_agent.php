...
/*{{{ Cpa-1 : Choix du statut juridique */
if ($global_nom_ecran == "Cpa-1") {
    $SESSION_VARS['statut_juridique'] = NULL;
    $SESSION_VARS['POSTED_DATAS'] = NULL;
    $myForm = new HTML_GEN2();
    $myForm->setTitle(_("Création client via agent: Renseignements sur le statut juridique"));
    ...
    if ($global_nom_ecran == "Cpa-2") {
        ...
        if ($global_nom_ecran_prec == "Cpa-1") {
            ...
            if ($global_nom_ecran_prec == "Cpa-2") {
                ...
                if ($global_nom_ecran_prec == "Cpa-3") {
                    foreach ($SESSION_VARS["POSTED_DATAS"] as $key => $fieldname)
                        $SESSION_VARS[$fieldname] = $SESSION_VARS["DATCpaI"][$fieldname];
                }

        $data_agc = getAgenceDatas($global_id_agence);

        // Création du formulaire
        $myForm = new HTML_GEN2(_("Création client via agent: Renseignement des données personnelles"));
    ...
    ".(intval($infoParamAbonnement['valeur']))."){\n";
      ...
      " . ($infoParamPrefixAbonnement['valeur']) . "){\n";
        ...
        if ($global_nom_ecran_prec == "Cpa-2" || $global_nom_ecran_prec == "Cpa-3") {
            foreach ($SESSION_VARS['POSTED_DATAS'] as $key => $fieldname) {
        ...
        if ($SESSION_VARS['statut_juridique'] == 4) {
            ...
            $myForm->setFormButtonProperties("ajout", BUTP_PROCHAIN_ECRAN, 'Cpa-2');
        ...
        $myForm->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, 'Cpa-3');
        $myForm->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, 'Cpa-1');
    ...
    /*{{{ Cpa-3 : Confirmation des données */
    else
        if ($global_nom_ecran == "Cpa-3") {
            global $double_affiliation;
            // Création du tableau $DATCpaI qui va contenir les données utiles à l'insertion du client
            $DATCpaI = array ();
            // Alimentation avec le statut juridique
            $DATCpaI["statut_juridique"] = $SESSION_VARS["statut_juridique"];

            // Ajout des données sur l'utilisateur créateur
            $DATCpaI["utilis_crea"] = $global_id_utilisateur;

            // Remplissage de $DATCpaI avec les données postées au serveur.
            $POSTED_DATAS = $SESSION_VARS["POSTED_DATAS"];

            foreach ($POSTED_DATAS as $key => $fieldname) {
                if ($fieldname == 'photo')
                    $DATCpaI[$fieldname] = $SESSION_VARS['DATCpaI']['photo'];
                elseif ($fieldname == 'signature') $DATCpaI[$fieldname] = $SESSION_VARS['DATCpaI']['signature'];
                else
                    $DATCpaI[$fieldname] = $ {$fieldname};
            }
            //Contrôle sur la double affiliation
            if(($SESSION_VARS["statut_juridique"] == 1) && (!$double_affiliation)){
                $msg = "";
                $Num_piece = getNumPieceId($DATCpaI['pp_type_piece_id']);
                // Vérifier que le numéro de la pièce d'identité n'est pas déjà utilisé
                for($i = 0;$i < count($Num_piece);$i++){
                    if($DATCpaI['pp_nm_piece_id'] == $Num_piece[$i]["pp_nm_piece_id"]){
                ...
                if ($msg != "") {
                    ...
                    $MyPage->addButton(BUTTON_OK, "Cpa-2");
                    $MyPage->buildHTML();
            ...
            if (isset ($IMAGES))
                foreach ($IMAGES as $imageName => $imagePath)
                    $DATCpaI[$imageName] = $imagePath;

            if ($DATCpaI['id_client'] != NULL && client_exist($DATCpaI['id_client'])) {
                $myHtmlErr = new HTML_Erreur(_("Erreur sur le numéro de client"));
                $myHtmlErr->setMessage(sprintf(_("Le numéro de client %s est déjà utilisé par un autre client"), $DATCpaI['id_client']));
                $myHtmlErr->addButton("BUTTON_OK", 'Cpa-2');
                $myHtmlErr->buildHTML();
                ...
                if ($SESSION_VARS['statut_juridique'] == 4) {
                    ...
                    $myForm->addHTMLExtraCode("membres", "<table align=\"center\" valign=\"middle\" bgcolor=\"" . $colb_tableau . "\"><tr><td><b>"._("Membres du groupe")."</b></td></tr></table>\n");
                    for ($i = 1; $i <= $SESSION_VARS["nbr_membres_encodes"]; ++ $i) {
                        if (($DATCpaI["num_client$i"] != NULL) && (strlen($DATCpaI["num_client$i"]) > 0)) {
                            // vérification de l'etat des membres
                            $etat = getEtatClient($DATCpaI["num_client$i"]);
                            if ($etat != 2) {
                                $myHtmlErr = new HTML_Erreur(_("Erreur dans l'encodage des membres"));
                                if ($etat == NULL) {
                                    $myHtmlErr->setMessage(sprintf(_("Attention, le client %s n'existe pas !"), $DATCpaI["num_client$i"]));
                                } else {
                                    $myHtmlErr->setMessage(sprintf(_("Attention, le client %s n'est pas actif, son état actuel est %s."), $DATCpaI["num_client$i"], adb_gettext($adsys["adsys_etat_client"][$etat])));
                                }
                                $myHtmlErr->addButton("BUTTON_OK", 'Cpa-2');
                                $myHtmlErr->buildHTML();
                                echo $myHtmlErr->HTML_code;
                            ...
                            if($ag_data["nb_group_for_cust"] > 0){
                                $num_client = $DATCpaI["num_client$i"];
                        ...
                        $myForm->addField("num_client$i", _("Membre $i"), TYPC_INT);
                        $myForm->setFieldProperties("num_client$i", FIELDP_DEFAULT, $DATCpaI["num_client$i"]);
                ...
                foreach ($include as $key => $value) {
                    if ($value == "photo" || $value == "signature") { // Champs image ==> on ne reprend que le nom du fichier
                        global $http_prefix;
                        if (is_file($DATCpaI[$value])) {
                            $url = $http_prefix."/images_tmp/".basename($DATCpaI[$value]);
                        } else {
                            $url ="/adbanking/images/travaux.gif";
                        }
                        $myForm->setFieldProperties($value, FIELDP_IMAGE_URL, $url);
                    } else
                        $myForm->setFieldProperties($value, FIELDP_DEFAULT, $DATCpaI[$value]);
                }
                // Traitement spécial pour le casier judiciaire
                if ($DATCpaI['pp_casier_judiciaire'] == 'f')
                    $myForm->setFieldProperties("pp_casier_judiciaire", FIELDP_DEFAULT, false);

                // Traitement spécial pour le VIP
                if ($DATCpaI['pp_is_vip'] == 'f') {
                ...
                if(sizeof($SESSION_VARS['champsExtras'])> 0 ) {
                    ...
                    $DATCpaI["champsExtras"] = $SESSION_VARS['champsExtrasValues'];
                }
                /*if(isset($SESSION_VARS["champs_extras"])){
                    ...
                    foreach($SESSION_VARS["champs_extras"] AS $key => $valeur) {
                      ...
                      $myForm->setFieldProperties($valeur['code'], FIELDP_DEFAULT, $DATCpaI[$valeur['code']]);
                      $myForm->setFieldProperties($valeur['code'], FIELDP_IS_LABEL, true);
                      $champs_extras[$key] = $DATCpaI[$valeur['code']];
                      unset($DATCpaI[$valeur['code']]);
                    }
                    $DATCpaI["serial_champs_extras"] = serialize($champs_extras);
                }*/

                // Ajout des boutons
                $ad_agc = getAgenceDatas($global_id_agence);
                $next = ($ad_agc['bloc_crea_cli'] == 't')?'Cpa-7':'Cpa-4';

                if (!isset($myHtmlErr)) {
                    // Il n'y a pas eu de problème à la confirmation
                    $myForm->addFormButton(1, 1, "ok", _("Valider"), TYPB_SUBMIT);
                    $myForm->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, $next);
                    $myForm->addFormButton(1, 2, "precedent", _("Précédent"), TYPB_SUBMIT);
                    $myForm->setFormButtonProperties("precedent", BUTP_PROCHAIN_ECRAN, 'Cpa-2');
            ...
            $SESSION_VARS["DATCpaI"] = $DATCpaI;
        }
        /*}}}*/

        /*{{{ Cpa-4 : Perception versement initial */
        else
            if ($global_nom_ecran == "Cpa-4") {
                global $global_monnaie;
                setMonnaieCourante($global_monnaie);

                global $global_id_client;

                if($global_nom_ecran_prec == 'Eca-1'){
                    $SESSION_VARS['id_client'] = $_GET['id_client'];
                    $liste_dem_appro_crea = getClientDatasAgent(11, $SESSION_VARS['id_client']);
                    $SESSION_VARS['POSTED_DATAS'] = array_keys($liste_dem_appro_crea[0]);
                    $SESSION_VARS['DATCpaI'] = $liste_dem_appro_crea[0];
                    $SESSION_VARS['statut_juridique'] = $SESSION_VARS['DATCpaI']['statut_juridique'];
                    $SESSION_VARS['etat_cli_bloc'] = $_GET['etat_cli_bloc'];
                    $SESSION_VARS['nbr_membres_encodes'] = 3;
                    $SESSION_VARS['champsExtras'] = Array();
                }
                ...
                $SESSION_VARS["montant_frais_adhesion"]= getMontantDroitsAdhesion($SESSION_VARS["DATCpaI"]["statut_juridique"]);
                ...
                if( $global_type_structure == 1) {//groupe solidaire ne paye pas les part sociales
                    if( $SESSION_VARS["DATCpaI"]["statut_juridique"] == 4 AND $AGC["paiement_parts_soc_gs"]=='f')
                        $is_mutuel_ps =false;
        ...
        function setPaye(valeur)
      {
        ...
        montant = (" . $AGC["val_nominale_part_sociale"] . ");
                ...
                $Label = array ();
                if ($SESSION_VARS["DATCpaI"]["statut_juridique"] == 1) {
                    $cli_include = array (
                        "pp_nom",
                        "pp_prenom"
                    );
                    ...
                    if ($SESSION_VARS["DATCpaI"]["statut_juridique"] == 3 || $SESSION_VARS["DATCpaI"]["statut_juridique"] == 4) {
                        $cli_include = array (
                            "gi_nom"
                        );
                        ...
                        if ($SESSION_VARS["DATCpaI"]["statut_juridique"] == 2) {
                            $cli_include = array (
                                "pm_raison_sociale"
                            );
                    ...
                    $codejs128 = "function verifSous()
	{
            ...
            if ( nbre_ps_lib > 0 ){
                ...
                if (isset ($SESSION_VARS["ouvre_cpt_base"])) // On vient de l'écran Cpa-5
                ...
                if ($SESSION_VARS["DATCpaI"]["statut_juridique"] == 1) {
                    $myForm->setFieldProperties("pp_nom", FIELDP_DEFAULT, $SESSION_VARS["DATCpaI"]["pp_nom"]);
                    $myForm->setFieldProperties("pp_prenom", FIELDP_DEFAULT, $SESSION_VARS["DATCpaI"]["pp_prenom"]);
                } else
                    if ($SESSION_VARS["DATCpaI"]["statut_juridique"] == 3 || $SESSION_VARS["DATCpaI"]["statut_juridique"] == 4) {
                        $myForm->setFieldProperties("gi_nom", FIELDP_DEFAULT, $SESSION_VARS["DATCpaI"]["gi_nom"]);
                    } else
                        if ($SESSION_VARS["DATCpaI"]["statut_juridique"] == 2) {
                            $myForm->setFieldProperties("pm_raison_sociale", FIELDP_DEFAULT, $SESSION_VARS["DATCpaI"]["pm_raison_sociale"]);
                        }
...
//                $myForm->addFormButton(1, 1, "rien", "      "._("Payera plus tard")."       ", TYPB_SUBMIT);
                $bouton_post = false;
                if ($SESSION_VARS["DATCpaI"]["statut_juridique"] == 1) {//Personne physique
                    if ($AGC['pp_montant_droits_adhesion']!=0) {
                        $myForm->addFormButton(1, 2, "post", "  "._("Paye les frais d'adhésion")."  ", TYPB_SUBMIT);
                        $bouton_post = true;
                    } else {
                        $myForm->addFormButton(1, 1, "post", "  "._("Versement initial")."  ", TYPB_SUBMIT);
                        $bouton_post = true;
                    }
                }
                if ($SESSION_VARS["DATCpaI"]["statut_juridique"] == 2) {    // Personne morale
                ...
                if ($SESSION_VARS["DATCpaI"]["statut_juridique"] == 3) { // Groupe informel
                ...
                if ($SESSION_VARS["DATCpaI"]["statut_juridique"] == 4) { // Groupe solidaire
                ...
                if ($is_mutuel_ps) {//pour une MEC -ps
                    ...
                    $myForm->setFormButtonProperties("tout", BUTP_PROCHAIN_ECRAN, 'Cpa-5');
                ...
                $myForm->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, 'Gen-3');
                if ($bouton_post==true)
                    $myForm->setFormButtonProperties("post", BUTP_PROCHAIN_ECRAN, 'Cpa-5');

//                $myForm->setFormButtonProperties("rien", BUTP_PROCHAIN_ECRAN, 'Cpa-6');
                //javascript boutons
...
//                $myForm->setFormButtonProperties("rien", BUTP_JS_EVENT, array ("onclick" => "setPaye(0);"));
            ...
            /*{{{ Cpa-5 : Enregistrement du versement initial */
            else
                if ($global_nom_ecran == 'Cpa-5') {
                    global $global_monnaie;
                    setMonnaieCourante($global_monnaie);
                    ...
                    $myForm->addFormButton(1, 1, "ok", _("Valider"), TYPB_SUBMIT);
                    $myForm->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, 'Cpa-6');
                    $myForm->addFormButton(1, 2, "retour", _("Précédent"), TYPB_SUBMIT);
                    $myForm->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, 'Cpa-4');
                    $myForm->setFormButtonProperties("retour", BUTP_CHECK_FORM, false);
                    $myForm->buildHTML();
                    echo $myForm->getHTML();
                ...
                /*{{{ Cpa-6 : Traitement DB et confirmation */
                else
                    if ($global_nom_ecran == "Cpa-6") {
                        global $global_id_client;
                        global $global_monnaie;
                        setMonnaieCourante($global_monnaie);

                        // si on vient directement de Cpa-4
                        if ($global_nom_ecran_prec == "Cpa-4") {
                            $SESSION_VARS["DATACPT"]["intitule_compte"] = $intitule_compte;
                        ...
                        if ($SESSION_VARS["DATCpaI"]["id_client"] == NULL) {
                            $global_id_client = getNewClientID();
                        } else {
                            $global_id_client = $SESSION_VARS["DATCpaI"]["id_client"];
                        }
                        $global_etat_client = 1; // A ce stade, le client est en cours de validation
                        $global_langue_rapport = $SESSION_VARS["DATCpaI"]['langue_correspondance'];

                        $SESSION_VARS["DATCpaI"]["id_client"] = $global_id_client;
                        $SESSION_VARS["DATCpaI"]["etat"] = $global_etat_client;
                        $SESSION_VARS["DATCpaI"]["nbre_parts"] = 0; // Nécessaire pour pouvoir additionner ensuite le nombre de parts souscrites
                        $SESSION_VARS["DATCpaI"]["nbre_parts_lib"] = 0;

                        $id_agc = getNumAgence();
                        $AGD = getAgenceDatas($id_agc);
                        $type_num_cpte = $AGD['type_numerotation_compte'];
                        //$SESSION_VARS["montant_frais_adhesion"] = getMontantDroitsAdhesion($SESSION_VARS["DATCpaI"]["statut_juridique"]);
                        //$DATCpaI = getClientDatas($global_id_client);
                        $AGC = getAgenceDatas($global_id_agence);
                        $mnt_min_cpt_base = recupMontant($SESSION_VARS["mnt_min"]);
                        if ($AGD["tranche_frais_adhesion"] == "t") {
                            $SESSION_VARS["DATCpaI"]["solde_frais_adhesion_restant"] = $SESSION_VARS["montant_frais_adhesion"];
                        }
                        ...
                        if ($SESSION_VARS['DATCpaI']['statut_juridique'] == 1)
                            $global_client = $SESSION_VARS['DATCpaI']['pp_prenom'] . ' ' . $SESSION_VARS['DATCpaI']['pp_nom'];
                        else
                            if ($SESSION_VARS['DATCpaI']['statut_juridique'] == 2)
                                $global_client = $SESSION_VARS['DATCpaI']['pm_raison_sociale'];
                            else // statut_juridique 3 ou 4
                                $global_client = $SESSION_VARS['DATCpaI']['gi_nom'];
                        ...
                        // personne physique
                        if (array_key_exists('pp_nom', $SESSION_VARS["DATCpaI"]) && $SESSION_VARS["DATCpaI"]["pp_nom"] !== '' ) {
                            $SESSION_VARS["DATCpaI"]["pp_nom"] = trim(preg_replace('/\s+/',' ', $SESSION_VARS["DATCpaI"]["pp_nom"]));
                        }
                        if (array_key_exists('pp_prenom', $SESSION_VARS["DATCpaI"]) && $SESSION_VARS["DATCpaI"]["pp_prenom"] !== '' ) {
                            $SESSION_VARS["DATCpaI"]["pp_prenom"] = trim(preg_replace('/\s+/',' ', $SESSION_VARS["DATCpaI"]["pp_prenom"]));
                        }

                        // personne morale
                        if (array_key_exists('pm_raison_sociale', $SESSION_VARS["DATCpaI"]) && $SESSION_VARS["DATCpaI"]["pm_raison_sociale"] !== '' ) {
                            $SESSION_VARS["DATCpaI"]["pm_raison_sociale"] = trim(preg_replace('/\s+/',' ', $SESSION_VARS["DATCpaI"]["pm_raison_sociale"]));
                        }

                        // groupe informelle or groupe solidaire
                        if (array_key_exists('gi_nom', $SESSION_VARS["DATCpaI"]) && $SESSION_VARS["DATCpaI"]["gi_nom"] !== '' ) {
                            $SESSION_VARS["DATCpaI"]["gi_nom"] = trim(preg_replace('/\s+/',' ', $SESSION_VARS["DATCpaI"]["gi_nom"]));
                        }

                        $SESSION_VARS["DATCpaI"]['is_login_agb'] = true;
                        $myErr = creationClientAgent($SESSION_VARS["DATCpaI"], $SESSION_VARS["DATACPT"], $SESSION_VARS["paye"], $SESSION_VARS["ouvre_cpt_base"], $versement, $SESSION_VARS['nbre_parts'],$SESSION_VARS['nbre_parts_lib'],$SESSION_VARS['somme'] , $global_id_guichet, $SESSION_VARS["transfert_client"], $data_ext, $SESSION_VARS["banque"], $SESSION_VARS["montant_frais_adhesion"], $SESSION_VARS["nbr_membres_encodes"], $SESSION_VARS['etat_cli_bloc']);
                        debug($myErr,"my error");
                        $id_his_creation = $myErr->param;
                        $DATCpaI = getClientDatas($global_id_client);
                        ...
                        if ($myErr->errCode != NO_ERR) {
                            ...
                            $data_comm = getCommissionNouveauClient();
                            if(sizeof($data_comm) > 0 && $data_comm['montant_comm'] >= 0) {
                                $err_comm_agent = commissionAgentSurNouveauxClient($data_comm,$id_his_creation);

                                if ($err_comm_agent->errCode != NO_ERR) {
                                    $html_err = new HTML_erreur(_("Echec de la commission agent sur nouveaux client."));
                                    $html_err->setMessage(_("Erreur") . " : " . $error[$err_comm_agent->errCode] . "<br/>" . _("Paramètre") . " : " . $err_comm_agent->param);
                                        ...
                                        if ($SESSION_VARS["paye"] == 2) {
                                            ...
                                            if ($AGD["tranche_part_sociale"] == "t") {
                                                ...
                                                if ($SESSION_VARS["nbre_parts"] > 0) {
                                                    $err = souscriptionPartsSocialesInt($SESSION_VARS["DATCpaI"]["id_client"], $SESSION_VARS["nbre_parts"], $global_id_utilisateur);
                                                    if ($err->errCode == NO_ERR) {
                                                        $err_update = updateSodeRestantPartSoc($SESSION_VARS["DATCpaI"]["id_client"], $montant_souscription);
                                                        if ($err_update->errCode != NO_ERR) {
                                                            ...
                                                            $id_his = $err->param;
                                                            $err_h = historique_mouvementPs($SESSION_VARS["DATCpaI"]["id_client"], $id_his, 20);
                                            ...
                                            if ($AGD ["tranche_part_sociale"] == "t") {
                                                if (recupMontant($SESSION_VARS['somme']) > 0) {
                                                    $err = liberationPartsSocialesInt($SESSION_VARS["DATCpaI"]["id_client"], $SESSION_VARS["nbre_parts_lib"], $global_id_utilisateur, recupMontant($SESSION_VARS['somme']));

                                                    // MAJ solde restant
                                                    if ($err->errCode == NO_ERR) {
                                                        $err_update = updateSodeRestantPartSoc($SESSION_VARS["DATCpaI"]["id_client"], $montant_part_soc_restant);
                                                        if ($err_update->errCode != NO_ERR) {
                                                            $html_err = new HTML_erreur (_("Echec de la mise à jour du solde restant."));
                                                            $html_err->setMessage(_("Erreur") . " : " . $error [$err_update->errCode] . "<br/>" . _("Paramètre") . " : " . $err_update->param);
                                                            $html_err->addButton("BUTTON_OK", 'Gen-3');
                                                            ...
                                                            $id_his = $err->param;
                                                            $err_h = historique_mouvementPs($SESSION_VARS["DATCpaI"]["id_client"], $id_his, 28);
                                                            if ($err_h->errCode != NO_ERR) {
                                                                return $err_h;
                                                            }
                                                        }
                                                ...
                                                if ($SESSION_VARS['nbre_parts_lib'] > 0) {
                                                    $err = liberationPartsSocialesInt($SESSION_VARS["DATCpaI"]["id_client"], $SESSION_VARS['nbre_parts_lib'], $global_id_utilisateur);
                                    ...
                                    //$SESSION_VARS["DATCpaI"]["statut_juridique"]
                                    ...
                                    $msg .= "<br/><br/>" . sprintf(_("La commission agent '%s' pour la creation du nouveau client est '%s'"), $global_nom_login, afficheMontant($data_comm['montant_comm'], true));
...
//                            $myForm->addCustomButton("menu", _("Aller au menu clientèle"), 'Gen-9');
                                    $myForm->addCustomButton("butVal", _("Ok"), 'Gen-16');

                                    $myForm->buildHTML();
                                    echo $myForm->HTML_code;
                                }
                            }else{
                                $erreur = new HTML_erreur(_("Commission sur nouveau client"));
                                $erreur->setMessage(_("Le commission pour agent sur la creation d'un nouveau client n'est pas paramétré"));
                    ...
                    }else if ($global_nom_ecran == "Cpa-7") {
                        global $global_id_agence, $dbHandler, $global_id_client, $global_nom_login;
                        ...
                        $SESSION_VARS["DATCpaI"]["id_client"] = $global_id_client;
                        $SESSION_VARS["DATCpaI"]['etat'] = 10;
                        $SESSION_VARS["DATCpaI"]['is_login_agb'] = true;
                        $SESSION_VARS['nbr_membres_gs'] = 3;
                        $data_cli = $SESSION_VARS["DATCpaI"];
                        $IMAGES = array (
                            "photo" => $data_cli["photo"],
                            "signature" => $data_cli["signature"]
                        );
                        unset ($data_cli["photo"]);
                        unset ($data_cli["signature"]);

                        //Récuperer les champs supplémentaire
                        if( isset($data_cli["champsExtras"]) ) {
                            $ChampsExtras = $data_cli["champsExtras"];
                            unset ($data_cli["champsExtras"]);
                        }
                        $data_cli['id_ag']= $global_id_agence;
                        $ad_agc = getAgenceDatas($global_id_agence);

                        if(!isset($blocage_autoriser) && ($ad_agc['bloc_crea_cli'] == 't') ) {
                            $html_msg = new HTML_message("Demande autorisation du creation d'un nouveau client");

                            $html_msg->setMessage("<center><span style='color: #FF0000;'>Veuillez choisir une option ci-dessous ?<br /><br/></center><input type=\"hidden\" name=\"blocage_autoriser\" value=true />");
                            $html_msg->addCustomButton("btn_demande_autorisation_creation_client", "Demande d’autorisation", 'Cpa-7');
                            $html_msg->addCustomButton("btn_annuler", "Annuler", 'Gen-16');
                            ...
                            die();
                                ...
                                if ($data_cli["statut_juridique"] == 4 && $SESSION_VARS['nbr_membres_gs'] > 0) {
                                    $membres_gs = array();
                                    $nbr_membres_enregistres = 0;
                                    for ($i = 1; $i <= $SESSION_VARS['nbr_membres_gs']; ++$i) {
                                ...
                                $result = executeQuery($db, buildInsertQuery("ad_cli", $data_cli));
                                if (DB::isError($result)) {
                                    $dbHandler->closeConnection(false);
                                    signalErreur(__FILE__, __LINE__, __FUNCTION__, $result->getMessage());
                                } else {
                                    $dbHandler->closeConnection(true);
                                    $html_msg = new HTML_message("Blocage creation client par agent");
                                ...
                                $myErr = ajout_historique(30, $data_cli["id_client"], "", $global_nom_login, date("r"));
