...
//-----------------------------------------------------------------
//------- Ecran Rda-1 Choix du compte et du type de retrait -------
//-----------------------------------------------------------------
if ($global_nom_ecran == "Rda-1") {
  ...
  $html = new HTML_GEN2();
  $html->setTitle(_("Retrait en déplacé via agent sur un compte : choix du compte"));
  ...
  $html->setFieldProperties("type_retrait1", FIELDP_IS_REQUIRED, true);
  $choix2 = array();
  //les clés des choix correspondent à la table $adsys['adsys_type_piece_payement']
  $choix2[1] = _('Retrait Cash avec impression reçu');

  $html->setFieldProperties("type_retrait1", FIELDP_DEFAULT, 'Retrait Cash avec impression reçu');
  $html->setFieldProperties("type_retrait1", FIELDP_IS_LABEL, true);

  //ordonner les champs
  $html->setOrder(NULL, array("htm1", "NumCpte", "libel", "devise", "etat_cpte", "duree_min_retrait_jour", "htm2", "type_retrait1"));
  ...
  $html->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, 'Rda-2');
...
else if ($global_nom_ecran == "Rda-2"){
  ...
  //Enregistrement des informations postées en God-1
  if (isset($NumCpte)) {
...
//                    }
  ...
  $html->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, 'Rda-3');
  $html->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, 'Rda-1');
  $html->setFormButtonProperties("cancel", BUTP_PROCHAIN_ECRAN, 'God-3');
...
else if ($global_nom_ecran == "Rda-3"){
  ...
  if (!$erreurGuichet) {
    ...
    $CheckOnLoadRetrait = "
           ...
           document.ADForm.od_comm_retrait.value = Math.ceil(mnt_retrait * ".$InfoCpte['comm_retrait_od'].");
    ...
    $html->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, 'Rda-4');
    $html->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, 'Rda-2');
    $html->setFormButtonProperties("cancel", BUTP_PROCHAIN_ECRAN, 'God-3');
    ...
    $html_err->addButton("BUTTON_OK", "Rda-2");
    $html_err->buildHTML();
...
else if ($global_nom_ecran == "Rda-4"){
  ...
  $data_cheque["commission_agent"] = $SESSION_VARS['commission_agent'];
  $data_cheque["commission_institution"] = $SESSION_VARS['commission_institution'];
  ...
  try {
    ...
    $AuditObj->insertTransacData($global_nom_login, $global_id_agence, $global_remote_id_agence, $global_remote_id_client, $SESSION_VARS["NumCpte"], 'retrait', $SESSION_VARS['type_retrait'], Divers::getTypeTransactionChoixLibel('retrait', $SESSION_VARS['type_retrait']), $SESSION_VARS["mnt"], serialize($SESSION_VARS), $SESSION_VARS["devise"],0,null,$commission_ope_deplace,$data_cheque["commission_agent"] ,$data_cheque["commission_institution"] );

    // Mouvement des comptes avec gestion des frais d'opérations sur compte s'il y lieu
    $erreur_remote = Retrait::retraitCpteRemoteAgent($pdo_conn, $global_remote_id_agence, $global_id_guichet, $SESSION_VARS["NumCpte"], $InfoProduit, $InfoCpte, $SESSION_VARS["mnt"], $SESSION_VARS['type_retrait'], $SESSION_VARS['id_mandat'], $data_cheque, NULL, $dataBef); // Multi-devise : $CHANGE

    if ($erreur_remote->errCode == NO_ERR) {

      $erreur_local = Retrait::retraitCpteLocalAgent($global_remote_id_agence, $global_id_guichet, NULL, $InfoProduit, $InfoCpte, $SESSION_VARS["mnt"], $SESSION_VARS['type_retrait'], NULL, $data_cheque, NULL, NULL); // Multi-devise : $CHANGE
  ...
  if ($erreur_remote->errCode == NO_ERR && $erreur_local->errCode == NO_ERR) {
    ...
    switch ($SESSION_VARS['type_retrait']) {
      case 1:
        Recu::printRecuRetraitAgent($pdo_conn, $global_remote_id_agence, $global_remote_id_client, $global_remote_client, $InfoProduit, $infos, $SESSION_VARS['mnt'], $erreur_remote->param['id_his'], 'REC-RRA', $SESSION_VARS['id_mandat'], $SESSION_VARS["remarque"], $SESSION_VARS["communication"], $SESSION_VARS['id_pers_ext'], NULL, $SESSION_VARS['denomination_conj'], $listTypesBilletArr, $valeurBilletArr, $global_langue_rapport, $total_billetArr, true,$commission_ope_deplace,$data_cheque['commission_agent'],$data_cheque['commission_institution']);
        break;
      case 15:
      case 4:
    ...
    $mntDebit = $SESSION_VARS['mnt'] + $InfoProduit['frais_retrait_cpt']+ $data_cheque["commission_od_retrait"] + $data_cheque["commission_agent"] + $data_cheque["commission_institution"] ;
    $message = "
        ...
        <td>" . Divers::afficheMontant($commission_ope_deplace, true) . "</td>
        </tr>
        <tr><td>" . _("Commission pour agent") . " : </td>
        <td>" . Divers::afficheMontant($data_cheque["commission_agent"], true) . "</td>
        </tr>
        <tr><td>" . _("Commission pour institution") . " : </td>
        <td>" . Divers::afficheMontant($data_cheque["commission_institution"], true) . "</td>
        </tr>";
    ...
    $html_msg->addButton("BUTTON_OK", 'God-3');
    $html_msg->buildHTML();
    echo $html_msg->HTML_code;
    ...
    $html_err->addButton("BUTTON_OK", 'God-3');
    $html_err->buildHTML();
    echo $html_err->HTML_code;
  }
