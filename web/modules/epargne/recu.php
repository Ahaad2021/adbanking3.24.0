...
function print_recu_depot($id_client, $nom_client, $mnt, $InfoProduit, $infos, $id_his, $id_pers_ext = NULL,$remarq=NULL,$communic=NULL, $mnt_frais_attente = 0, $id_mandat = NULL, $listTypesBilletArr = array(), $valeurBilletArr = array(), $global_langue_rapport, $total_billetArr = array(), $hasBilletage = false, $isBilletageParam) {
  ...
  //Génération du XSL-FO (grâce au XSLT) et du PDF (grâce à FOP)
    ...
    if($isBilletageParam == 't'){
        ...
        if($format_A5){
            $fichier_pdf = xml_2_xslfo_2_pdf($xml, 'recuA5.xslt', false, "recu_depot_".rand());
        } else {
            $fichier_pdf = xml_2_xslfo_2_pdf($xml, 'recu.xslt', false, "recu_depot_".rand());
        }
    }
    else{
        if($format_A5){
            $fichier_pdf = xml_2_xslfo_2_pdf($xml, 'recuA5_ancien.xslt', false, "recu_depot_".rand());
        } else {
            $fichier_pdf = xml_2_xslfo_2_pdf($xml, 'recu_ancien.xslt', false, "recu_depot_".rand());
        }
    }
...
// $info_cheque->new_child("banque_cheque", getLibel("adsys_banques", $id_bqe));
  ...
  //Génération du XSL-FO (grâce au XSLT) et du PDF (grâce à FOP)
  if($format_A5){
  	$fichier_pdf = xml_2_xslfo_2_pdf($xml, 'recuA5.xslt', false,"recu_retrait_".rand());
  } else {
  	$fichier_pdf = xml_2_xslfo_2_pdf($xml, 'recu.xslt', false,"recu_retrait_".rand());
  }
 ...
 //montant en lettre
  ...
  //Génération du XSL-FO (grâce au XSLT) et du PDF (grâce à FOP)
    if($isBilletageParam=='t'){
        ...
        if($format_A5){
            $fichier_pdf = xml_2_xslfo_2_pdf($xml, 'recuA5_ancien.xslt', false, "recu_retrait_".rand());
        } else {
            $fichier_pdf = xml_2_xslfo_2_pdf($xml, 'recu_ancien.xslt', false, "recu_retrait_".rand());
        }
    }
