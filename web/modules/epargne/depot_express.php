...
if ($global_nom_ecran == "Dex-1") {
  ...
  echo $html->getHTML();
    ...
    global $global_id_client, $global_id_agence, $report_backup_path;
// capturer des types de billets de la bd et nombre de billets saisie par l'utilisateur
  ...
  $tarification_dataset = getTarificationDatas("SMS_FRAIS");
  $tarification = empty($tarification_dataset)?0:$tarification_dataset["valeur"];
  ...
  if ( check_access(299) ) {
      ...
      $montant_remboursement -= $InfoProduit["frais_depot_cpt"]; //EVOL MOBILE LENDING
  }
...
/*Pending en attendant evol pour les salariés/ commercants
  ...
  if(isMobileLending()) {
      ...
      if ($rembourse_erreur->errCode != NO_ERR) {
          ...
          $html_err->addButton("BUTTON_OK", 'Dex-1');
          $html_err->buildHTML();
          echo $html_err->HTML_code;
          exit();
      }
