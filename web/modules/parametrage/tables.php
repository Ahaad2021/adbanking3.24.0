...
if(isFusion() == FALSE){
    unset($tables["adsys_niveau_agence"]);
}
asort($tables);
...
// ecran de valisation des donnees pour les localisations Rwanda
  ...
  if (isset($butval) && $butval == "Valider") {
    ...
    $MyPage = new HTML_GEN2(_("Gestion de la table de paramétrage") . " Niveau Agence");
    //Si table générique
    ...
    $MyPage->addField("contenu", _("Niveau agence"), TYPC_LSB);
    $MyPage->setFieldProperties("contenu", FIELDP_HAS_CHOICE_AUCUN, true);
    $MyPage->setFieldProperties("contenu", FIELDP_IS_REQUIRED, true);
    $liste_localisationFENACOBU = getListelocalisationFenacobu("type_niveau <> 1 ");
    //Trier par ordre alphabétique
    ...
    if (isset($butajou) && $butajou = 'Ajouter') {
        $MyPage = new HTML_GEN2(_("Ajout niveau agence"));
        ...
        $codejs = " function populateParent()
      {
        ...
        unset($adsys["type_niveau_agence"][1]);
        $MyPage->addJS(JSP_FORM, "JS1", $codejs);
        ...
        //$MyPage->setFormButtonProperties("butval", BUTP_JS_EVENT, array("onclick" => $js_control));
        $MyPage->addFormButton(1, 2, "butret", _("Annuler"), TYPB_SUBMIT);
        $MyPage->setFormButtonProperties("butret", BUTP_CHECK_FORM, false);
        ...
        if (document.ADForm.HTML_GEN_LSB_type_niveau.value > 1) {
            ...
            $codejs_remove = "
              $(document).ready(function(){
                $('[name=HTML_GEN_LSB_type_niveau] option:nth-child(2)').remove();
                console.log($('[name=HTML_GEN_LSB_type_niveau] option:nth-child(2)'));
              });
            ";
            $MyPage->addJS(JSP_FORM, "JS_remove", $codejs_remove);
            $MyPage->addJS(JSP_FORM, "JS1", $codejs);
//            $MyPage->setFieldProperties('type_niveau', FIELDP_TYPE, TYPC_LSB);
//            $MyPage->setFieldProperties("type_niveau", FIELDP_HAS_CHOICE_AUCUN,true);
//            $MyPage->setFieldProperties('type_niveau', FIELDP_ADD_CHOICES, $adsys["type_niveau_fenacobu"]);
    ...
    if (isset($butval) && $butval == "Valider") {
        ...
        //Verifier si le contenu a une dependance
        $where_dependance = " parent = $contenu$contenu";
        $check_dependance = getLocalisationFENACOBUDetails($where_dependance);
        if (sizeof($data_cli)>0) {
            $boolean_check = false;
            $erreur = new HTML_erreur(_("Dependance associé"));
            $erreur->setMessage(_("Le contenu que vous voulez supprimer possède des dépendances!"));
        ...
        if ($check_niveau[$contenu]["type_niveau"] == 1){
            $boolean_check = false;
            $erreur = new HTML_erreur(_("Impossible de supprimer la coopec"));
            $erreur->setMessage(_("Vous ne pouvez pas supprimer une coopec!"));
            $erreur->addButton(BUTTON_OK,"Lob-1");
            $erreur->buildHTML();
            echo $erreur->HTML_code;
            $ok = false;

        }else if ($check_niveau[$contenu]["type_niveau"] == 2){
            $conditionCliWhere = " n_agence = ".$check_niveau[$contenu]["id"];
            $data_cli = getClientFENACOBUDetails($conditionCliWhere);
            if (sizeof($data_cli)>0){
                $boolean_check = false;
                $erreur = new HTML_erreur(_("Client associé"));
                $erreur->setMessage(_("L' agence que vous voulez supprimer est rattaché a un client!"));
                $erreur->addButton(BUTTON_OK,"Lob-1");
                $erreur->buildHTML();
                echo $erreur->HTML_code;
                $ok = false;
        ...
        }else if ($check_niveau[$contenu]["type_niveau"] == 3){
            $conditionCliWhere = " n_guichet = ".$check_niveau[$contenu]["id"];
            $data_cli = getClientFENACOBUDetails($conditionCliWhere);
            if (sizeof($data_cli)>0){
                $erreur = new HTML_erreur(_("Client associé"));
                $erreur->setMessage(_("Le guichet que vous voulez supprimer est rattaché a un client!"));
                $erreur->addButton(BUTTON_OK,"Lob-1");
                $erreur->buildHTML();
                echo $erreur->HTML_code;
                $ok = false;
        ...
        if ($boolean_check == true){
            $del_data = deleteData("adsys_niveau_agence", array("id" => $check_niveau[$contenu]["id"]));
            if ($myErr->errCode == NO_ERR) {
