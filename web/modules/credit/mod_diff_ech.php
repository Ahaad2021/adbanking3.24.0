 ...
 * - Dec-1 : sélection d'un dossier de crédit
 * - Dec-2 : échéancier actuel à modifier
 * - Dec-3 : nouvel échéancier
 * - Dec-4 : confirmation enregistrement
 *
 * @package Credit
 */
...
if ($global_nom_ecran == "Dec-1") {
    ...
    $cre_etat_all = array_keys(getTousEtatCreditAll());
    $whereCl = " AND (etat=5 AND cre_etat IN (".implode(', ', $cre_etat_all).")) ";
    ...
    if ($SESSION_VARS['infos_client']['statut_juridique'] == 4) {
        ...
        foreach($dossiers_fictifs as $id=>$value) {
            ...
            foreach($dossiers_membre as $id_doss=>$val)
                if (($val['id_dcr_grp_sol'] == $id) AND ($val['etat'] == 5) AND (in_array($val['cre_etat'], array($cre_etat_all)))) {
                    //if (allowed_demande_reech($id_doss)) {
    ...
    $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dec-2");
    $Myform->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);

    $Myform->setOrder(NULL,$order);
    $Myform->buildHTML();
    echo $Myform->getHTML();
}elseif ($global_nom_ecran == 'Dec-2') {
    ...
    if ($SESSION_VARS['dossiers'][$HTML_GEN_LSB_id_doss]['gs_cat'] != 2 ) { // dossier individuel
        ...
        $SESSION_VARS['infos_doss'][$id_doss]['ech_date_diff'] = date("d/m/Y");
    ...
    $differe_max = $SESSION_VARS['infos_prod']['differe_ech_max'];
    $JS_diff.= "
        function diff_check(){
            $('[name*=nmbr_diff_ech]').each(function(i){
                if($(this).val() == ''){
                   alert('- Nombre de differé en échéance n\'est pas renseigné\\n');
                   return false;
                }else if (parseInt($(this).val()) > parseInt(".$differe_max.")){
                   var id_doss = $(this).attr('name').match(/\d+/g);
                   alert('Le differé pour le dossier ' + id_doss + ' doit être inférieur ou égal à '+ $differe_max +' échéance(s)');
                   $(this).val('');
                   return false;
                }else if(($('[name*=nmbr_diff_ech]').length - 1) == i){
                   assign('Dec-3');
    ...
    $Myform->addJS(JSP_FORM,"diff_check",$JS_diff);
    // les boutons ajoutés
    ...
    // Propriétés des boutons
    $Myform->setFormButtonProperties("valider", BUTP_JS_EVENT, array('onclick' => 'diff_check();'));
    ...
    foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
        ...
        $SESSION_VARS["nmbr_diff_ech$id_doss"] = ${'nmbr_diff_ech'.$id_doss};
        ...
        if(empty($echeance_index)){
            $html_err = new HTML_erreur(_("Echec demande de differé en échéances ")." ");
            $html_err->setMessage(_("Le différé en échéances ne peut pas être ajouté car toutes les échéances sont dues"));
        ...
        for($pos = 0; $pos < ${'nmbr_diff_ech'.$id_doss}; $pos++){
...
//                $interets_attendus = calcEchInterest($id_prod, $capital, $duree_mois, $differe_jours, $differe_ech, NULL, $id_doss, $interet_actuelle);
//            }
//
...
//        }
    ...
    $formEcheancier->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dec-4");
...
elseif ($global_nom_ecran == "Dec-4") {
    ...
    foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
        ...
        if (is_array($DATA) && count($DATA)>0) {
            ...
            if(updateCredit($DATA["id_doss"], $UPDATE_DATA)) {
                ...
                if ($id_his == NULL) {
                    // Log action
                    $myErr = ajout_historique(103, $DATA["id_client"], "Demande de differé en échéances (".$DATA["id_doss"].")", $global_nom_login, date("r"), NULL);
                ...
                $erreur = $HisObj->insertDossierHis($DATA["id_doss"], 4, $DATA["echeance_index"], null, 0, $DATA["nmbr_echeance_differe"]);
    ...
    if ($has_error == FALSE) {
        $msg = new HTML_message(_("Confirmation demande de differé en échéances"));
