 ...
 * - Rde-1 : sélection d'un dossier de crédit
 * - Rde-2 : échéancier actuel à modifier
 * - Rde-3 : nouvel échéancier
 * - Rde-4 : confirmation enregistrement
 *
 * @package Credit
 */
...
// Récupération des infos du client (ou du GS)
    ...
    $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Rde-2");
...
/*{{{ And-2 : Affichage du dossier */
else if ($global_nom_ecran == "Rde-2") {
    // Si on vient de Apd-1, on récupère les infos de la BD
    if (strstr($global_nom_ecran_prec,"Rde-1")) {
    ...
    // Création du formulaire
    $Myform = new HTML_GEN2(_("Rejet de la demande de differé des échéance"));
...
// Test de la date d'annulation
    ...
    // Propriétés des boutons
    $Myform->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Gen-11");
    $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Rde-3");
    $Myform->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);
...
else if ($global_nom_ecran == "Rde-3") {
    ...
    foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
        ...
        if (is_array($DATA) && count($DATA)>0) {
            ...
            if (updateCredit($DATA["id_doss"], $UPDATE_DATA)) {
                if ($id_his == NULL) {
                    // Log action
                    $myErr = ajout_historique(108, $DATA["id_client"], "Rejet de differé en échéances (".$DATA["id_doss"].")", $global_nom_login, date("r"), NULL);
    ...
    if ($has_error == FALSE) {
        $msg = new HTML_message(_("Confirmation rejet de differé en échéances"));
        ...
        if (is_array($id_doss_arr) && count($id_doss_arr) > 0) {
            ...
            $msg->setMessage(_("Les Dossiers de crédit ".$list_id_doss." sont  passés avec succès à l'état rejeté !"));
        } else {
            $msg->setMessage(_("Le Dossier de crédit N°".$id_doss_arr[0]." est passé avec succès à l'état rejeté !"));
        }
        $msg->addButton(BUTTON_OK, "Gen-11");
        $msg->buildHTML();
        echo $msg->HTML_code;
    } else {
        $html_err = new HTML_erreur(_("Echec la demande de differé en échéances n'a pas été rejeté."));
