...
if ($global_nom_ecran == "Dbd-1") {
  ...
  $SESSION_VARS['screen_base_order'] = array(
      "Dbd-1" => true,
      "Dbd-11" => true,
      "Dbd-2" => true,
      "Dbd-3" => true,
...
/*{{{ Dbd-2 : Déboursement d'un crédit */
  ...
  if (strstr($global_nom_ecran_prec,"Dbd-11")) {
    ...
    /* Récupération des garanties déjà mobilisées pour ce dossier */
      $prelev_garantie = ($SESSION_VARS['infos_prod']['prelev_garanti'] == '1')?true:false;
      $SESSION_VARS['prelev_garanti'] = $prelev_garantie;
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss)
  {
    ...
    if($SESSION_VARS["is_cpte_remb"] == 't'){
        $Myform->addField("cpt_remb".$id_doss, _("Compte de remboursement"), TYPC_TXT);
        $Myform->setFieldProperties("cpt_remb".$id_doss,FIELDP_IS_LABEL,true);
        $cpt_remboursement = getAccountDatas($val_doss['cpt_remb']);
        $Myform->setFieldProperties("cpt_remb".$id_doss,FIELDP_DEFAULT,$cpt_remboursement["num_complet_cpte"]." ".$cpt_remboursement['intitule_compte']);
    }
...
/*{{{ Dbd-3 : Echéancier réel */
  ...
  $prelev_frais_comm_assurance = ($SESSION_VARS['infos_prod']["percep_frais_com_ass"]==1)?true:false;
  $prelev_garantie = ($SESSION_VARS['infos_prod']['prelev_garanti'] == '1')?true:false;

  if($prelev_frais_comm_assurance && $prelev_garantie){
      array_splice($SESSION_VARS["screen_base_order"], 4, 0, array("Dbd-4" => true));
      array_splice($SESSION_VARS["screen_base_order"], 8, 0, array("Dbd-7" => true));
  }elseif(!$prelev_frais_comm_assurance && $prelev_garantie){
      array_splice($SESSION_VARS["screen_base_order"], 4, 0, array("Dbd-4" => null, "Dbd-7" => true));
  }elseif($prelev_frais_comm_assurance && !$prelev_garantie){
      array_splice($SESSION_VARS["screen_base_order"], 7, 0, array("Dbd-7" => true, "Dbd-4" => null));
  }elseif(!$prelev_frais_comm_assurance && !$prelev_garantie){
      array_splice($SESSION_VARS["screen_base_order"], 4, 0, array("Dbd-7" => true, "Dbd-4" => null));
  }

  //preserving lost keys on splice function
  if($prelev_garantie){
      array_replace_key($SESSION_VARS["screen_base_order"], 0, "Dbd-4");
      array_replace_key($SESSION_VARS["screen_base_order"], 1, "Dbd-7");
  }else{
      array_replace_key($SESSION_VARS["screen_base_order"], 1, "Dbd-4");
      array_replace_key($SESSION_VARS["screen_base_order"], 0, "Dbd-7");
  }
...
//        else if ($is_gar_num) {
//            if (!$prelev_garantie) {
//                $formEcheancier->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dbd-7"); //Transfert des garanties
//                $SESSION_VARS['nxt_ecran'] = '7';
//            }else {
...
//    }
//  else {
//        if ($is_gar_num && $prelev_garantie) {
//            unset($SESSION_VARS['nxt_ecran']);
//            $formEcheancier->setFormButtonProperties("valider",BUTP_PROCHAIN_ECRAN,"Dbd-4"); //Transfert des garanties
//        } else{
//            $formEcheancier->setFormButtonProperties("valider",BUTP_PROCHAIN_ECRAN,"Dbd-7"); //Transfert des fonds
//            $SESSION_VARS['nxt_ecran'] = 'Dbd-7';
//        }
//  }

  $formEcheancier->setFormButtonProperties("valider",BUTP_PROCHAIN_ECRAN,getNextScreen($SESSION_VARS["screen_base_order"]));
 ...
 foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss)
 {
    ...
    //solde du compte de prélèvement de frais de dossiers
     if($prelev_garantie) {
         ...
         if(!empty($SESSION_VARS['screen_base_order']['Dbd-4'])){
             $soldeB = recupMontant(${'solde'.$id_doss.'_'.$_POST['key_'.$id_doss]});
             $formConf->addHiddenType('solde'.$id_doss.'_'.$_POST['key_'.$id_doss], $soldeB);
             $formConf->addHiddenType('key_'.$id_doss, $_POST['key_'.$id_doss]);
         }else{
             $soldeB = recupMontant(${'soldeF'.$id_doss});
         }
     }
   ...
   if (($SESSION_VARS['infos_doss'][$id_doss]["mnt_frais"] + $frais_values['mnt_tax_frais']) > $soldeB) {
    ...
    $formConf->setFieldProperties("soldeF".$id_doss,FIELDP_IS_READONLY,true);
    ...
    foreach($val_doss['DATA_GAR'] as $key=>$value ) {
      if ($value['type'] == 1 ) {
        ...
        if($prelev_garantie){
              ...
              $solde_avt = recupMontant(${'soldeF'.$id_doss});
              $soldeCompteGar = $solde_avt - $value["valeur"];
        }
        ...
        $formConf->addHiddenType('key_'.$id_doss, $key);
        ...
        $formConf->setFieldProperties("solde".$id_doss."_".$key,FIELDP_IS_READONLY,true);
...
//    if(!empty($SESSION_VARS['nxt_ecran']) && ($SESSION_VARS['nxt_ecran'] == 'Dbd-7')){
//        $formConf->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dbd-8"); // Transfert des des fonds
//    }
//    elseif ($is_frais_doss) {
//        $formConf->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dbd-10"); //Frais de dossier
//    } elseif ($is_commission) {
//        $formConf->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dbd-7"); //Commission
//    } elseif ($is_assurance) {
//        $formConf->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dbd-6"); //Assurances
//    }else{
//        $formConf->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dbd-4"); // Transfert des des fonds
//    }
//  } else { //apres deboursement
//        if(!empty($SESSION_VARS['nxt_ecran']) && ($SESSION_VARS['nxt_ecran'] == 'Dbd-7')){
//            $formConf->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dbd-5"); // Transfert des des fonds
//        }else{
//            $formConf->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dbd-7"); // Transfert des des fonds
//        }
//  }

  $formConf->setFormButtonProperties("valider",BUTP_PROCHAIN_ECRAN,getNextScreen($SESSION_VARS["screen_base_order"]));
  $formConf->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Gen-11");
  $formConf->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);
...
//      }else{
//          $SESSION_VARS['nxt_ecran'] = 'Dbd-7';
//          $formConf->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dbd-4"); // confirmation Transfert des garantie
//      }
//  }
//   else
//   {
...
//         }else{
//             $SESSION_VARS['nxt_ecran'] = 'Dbd-7';
//             $formConf->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dbd-4"); //Transfert garantie
//         }
//   	 }elseif ($is_assurance == true){
//		   $formConf->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Dbd-6"); //Assurances
//   	 }else {
...
/*{{{ Dbd-8 : Confirmation */
  ...
  if($prelev_garantie){
      ...
      $myErr = deboursementCreditAvantGarantie($SESSION_VARS['infos_doss'], $SESSION_VARS['mode_debour'], $SESSION_VARS['dest_debour'], $global_id_guichet, $prelev_garantie);
  }
  ...
  if ($myErr->errCode == NO_ERR) {
    ...
    $ml_demande_credit = current(getDemandeCredit($global_id_client, " id_doss = $id_doss"));

      if ($flag_mob_lending == 't'){
          global $dbHandler;
          // mise a jour statut de la demande a 5
          $db = $dbHandler->openConnection();
          $array_update = array("statut_demande" => 5);
          $array_condi = array("id_client" => $SESSION_VARS['infos_doss'][$id_doss]['id_client'], "id_transaction" => $ml_demande_credit['id_transaction']);
        ...
        // MSQ mobile lending
        // Checker si ce dossier est un premier credit et est mobile lending
        $premier_credit = isPremierCreditMobileLending($SESSION_VARS['infos_doss'][$id_doss]['id_client']);
        
        if ($premier_credit['premier_credit'] == 't'){
            $tx_interet = 0;
        } else {
            $tx_interet = $SESSION_VARS['infos_prod']['tx_interet'];
        }

	      if(isMSQEnabled() && $appli == 'main'){
            if (($info_prod[0]['crdt_salairie'] == 't') && ($info_prod[0]['is_mobile_lending_credit'] == 't')){
                envoi_sms_credit_mob_lending($SESSION_VARS['infos_client']['pp_prenom'], $SESSION_VARS['infos_doss'][$id_doss]['id_client'], $SESSION_VARS['infos_client']['id_cpte_base'], $SESSION_VARS['infos_doss'][$id_doss]['id_doss'], $SESSION_VARS['infos_doss'][$id_doss]['mnt_dem'], $tx_interet, 11, $SESSION_VARS['infos_doss'][$id_doss]['ECH'][1]['date_ech']);
            }else if (($info_prod[0]['is_mobile_lending_credit'] == 't') && ($info_prod[0]['crdt_salairie'] == 'f' )){
                envoi_sms_credit_mob_lending($SESSION_VARS['infos_client']['pp_prenom'], $SESSION_VARS['infos_doss'][$id_doss]['id_client'], $SESSION_VARS['infos_client']['id_cpte_base'], $SESSION_VARS['infos_doss'][$id_doss]['id_doss'], $SESSION_VARS['infos_doss'][$id_doss]['mnt_dem'], $tx_interet, 5, $SESSION_VARS['infos_doss'][$id_doss]['ECH'][1]['date_ech']);
            }

	      }
        // Fin MSQ mobile lending

          // mise a jour du champ premier credit a FALSE
          $db = $dbHandler->openConnection();
          $array_update_abo = array("premier_credit" => 'f');
          $array_condi_abo = array("id_client" => $SESSION_VARS['infos_doss'][$id_doss]['id_client'], "deleted" => 'f');
          $sql_abo = buildUpdateQuery("ad_abonnement", $array_update_abo, $array_condi_abo);

          $result_abo = executeDirectQuery($sql_abo);
          if ($result_abo->errCode != NO_ERR){
              $dbHandler->closeConnection(false);
              return new ErrorObj($result_abo->errCode);
          }
          $dbHandler->closeConnection(true);
      }
