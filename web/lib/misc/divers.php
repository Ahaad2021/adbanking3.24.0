...
/**
 * Fonction utilisée pour vérifier si l'agence est parametré commeagency banking dans licence2.txt
 * @author Ahaad M
 * @since 1.0
 */
function isAgencyBanking() {
  ...
  if (file_exists($licence2_path)) {
    ...
    // check engrais chimiques
    if(null !== $decrypte_arr[10] && strtolower(trim($decrypte_arr[10]))=='y') {
      $agency_banking = true;
    }
  }
...
/**
 * Fonction utilisée pour vérifier si l'agence est parametré comme nobile lending dans licence2.txt
 * @author Ahaad M
 * @since 1.0
 */
function isMobileLending() {

    global $dbHandler;

    $mobile_lending = false;
    ...
    if (file_exists($licence2_path)) {
        ...
        // check engrais chimiques
        if(null !== $decrypte_arr[11] && strtolower(trim($decrypte_arr[11]))=='y') {
            $mobile_lending = true;
        }
    }

    return $mobile_lending;
}

/**
 * Fonction utilisée pour vérifier si l'agence est parametré comme ATM module dans licence2.txt
 * @author Ahaad M
 * @since 1.0
 */
function isAtmModule() {
    ...
    if (file_exists($licence2_path)) {
        ...
        // check engrais chimiques
        if(null !== $decrypte_arr[12] && strtolower(trim($decrypte_arr[12]))=='y') {
            $module_atm = true;
        }
    }
...
/**
 * Fonction utilisée pour vérifier si l'agence est parametré comme fusion dans licence2.txt
 * @author Ahaad M
 * @since 1.0
 */
function isFusion() {
    ...
    if (file_exists($licence2_path)) {
        ...
        // check engrais chimiques
        if(null !== $decrypte_arr[13] && strtolower(trim($decrypte_arr[13]))=='y') {
            $fusion = true;
        }
    }
...
function getFiles($dir, $pattern){
    ...
    foreach($files as $key => $file){
        ...
        if(!empty($matches)){
            ...
            if(file_exists($dir.'/'.$file)){
                $files_metadata[$parts[count($parts) -1]] = array(
                    "file_name" => $file,
    ...
    ksort($files_metadata);
    return $files_metadata;
}

function updateTableliste($DATA, $where){
    global $dbHandler;
...
function getTableliste($nomc){
    global $dbHandler;
    $db = $dbHandler->openConnection();
    $sql = "select * from tableliste where nomc = '$nomc'";
    $result = $db->query($sql);
    if (DB :: isError($result)) {
        $dbHandler->closeConnection(false);
        signalErreur(__FILE__, __LINE__, __FUNCTION__);
...
function file_add_timestamp($dir, $pattern, $meta){
    global $dbHandler, $global_id_client, $global_id_agence;
    $files = getFiles($dir, $pattern);
    $file_timestamp = null;
    if(!empty($files)){
        $files_keys = array_keys($files);
        if(file_exists($files[$files_keys[0]]["relative_path"])){
            $file_timestamp = $files[$files_keys[0]]["relative_path"]."_".time();
            rename($files[$files_keys[0]]["relative_path"], $file_timestamp);
        }
    }

    //Due to AT-239, the database data related to this file need also to be updated
    $db = $dbHandler->openConnection();
    $meta_key = array_keys($meta);
    $file_path = empty($file_timestamp)?"adbanking/images/travaux.gif":(dirname($meta[$meta_key[0]])."/".basename($file_timestamp));
    $sql = "INSERT INTO ad_cli_hist (".$meta_key[0].", id_ag, id_client) VALUES ('".$file_path."', $global_id_agence, $global_id_client)";
