...
function getDernierEcheanceNonRemb($id_doss, $condition = null) {
  global $dbHandler,$global_id_agence;

  $db = $dbHandler->openConnection();

  $sql = "SELECT * FROM ad_etr WHERE (id_doss='$id_doss') AND remb='f' AND id_ag=$global_id_agence $condition ORDER BY id_ech asc LIMIT 1;";
 ...
 */
function insereDossier($DATA,$FRAIS,$GARANTIE,$id_utilisateur, $DOSSIERS_FICTIFS, $func_sys_ins_doss = 105, $pv = true) {
    ...
    if (is_array($GARANTIE)) {
      ...
      if ($gar_cli["benef"] == $id_cli)
        $gar_mobilisee[] = $gar_cli;
            $myErr = prepareGarantie($DOSS[$id_cli], $gar_mobilisee, $pv);
      if ($myErr->errCode != NO_ERR) {
        $dbHandler->closeConnection(false);
        return $myErr;
      }
 ...
 * @return ErrOject $myError un objet contenant le code d'erreur s'il y a erreur
 */
function supprimeDossier($source, $id_guichet, $DATA, $func_sys_correction_doss = 129, $func_sys_doss = 125) {
	...
	foreach($DATA as $id_doss=>$val) {
		...
		if ($val["etat"] == 5){
			$myErr = annulerDeboursementCredit($source, $id_guichet, $id_doss, $comptable_his, $func_sys_correction_doss, $func_sys_doss);
			if ($myErr->errCode != NO_ERR) {
				$dbHandler->closeConnection(false);
				return $myErr;
			}
 ...
 */
function prepareGarantie($id_doss, $DATA_GAR, $pv = true) {
  global $dbHandler,$global_id_agence;
  $db = $dbHandler->openConnection();
 ...
 */
function approbationCredit($info_doss, $pv = true, $func_sys_approb_doss = 110) {
      ...
      if (is_array($val_doss['DATA_GAR']))
        if (count($val_doss['DATA_GAR'] > 0)) {
          /* Blocage de toutes les garanties numéraires mobilisées */
                    $myErr = prepareGarantie($id_doss, $val_doss['DATA_GAR'], $pv);
          if ($myErr->errCode != NO_ERR) {
            $dbHandler->closeConnection(false);
            return $myErr;
          }
...
function deboursementCredit($infos_doss, $mode_debour, $dest_debour, $id_guichet, $func_sys_deb_doss = 125)
{
  ...
  foreach ($infos_doss as $id_doss => $val_doss) {
    ...
    if ($val_doss ['etat'] == 2) { // si c'est le premier déboursement du crédit
      ...
      if ($is_frais_doss && $PROD[0]['prelev_frais_doss'] == 2) {
 ...
 * @return ErreurObj renvoie un code d'erreur : 0 si pas erreur si non le code de l'erreur rencontrée
 */
function annulerDeboursementCredit($source, $id_guichet = NULL, $id_doss, &$comptable_his, $func_sys_correction_doss = 129, $func_sys_doss) {
  global $dbHandler, $global_id_agence, $global_nom_login;
  global $error;
  global $db;
    ...
    while ($tmpRow = $result_cpte_nantie->fetchrow(DB_FETCHMODE_ASSOC)){
      ...
      if ($infosCpteNantie != NULL) {
        ...
        if (!$is_cre_repris) { // credit mise en place
          ...
          $sql = "SELECT m.montant, m.devise from ad_his h, ad_ecriture e, ad_mouvement m where h.type_fonction = ".$func_sys_doss." and h.infos = '$id_doss' and m.cpte_interne_cli = $id_cpt_nantie and h.id_his=e.id_his ";
  ...
  // Recherche le montant et la devise pour le transfert des assurances
		$sql = "SELECT m.montant, m.devise from ad_his h, ad_ecriture e, ad_mouvement m where h.type_fonction = ".$func_sys_doss." and h.infos = '$id_doss' and h.id_his=e.id_his" ;
...
function rembourse ($id_doss, $mnt, $source, &$comptable, $id_guichet = NULL, $DATA_REMB = NULL, $ORDRE_REMB = NULL, $ech_paye = NULL, $date_remb = NULL, $id_cpte_gar = NULL, $DCR = NULL, $Produitx = NULL, $DEV = NULL, $array_credit = NULL, $cpta_debit = NULL, $cpta_credit_gar = NULL, $CPTS_ETAT = NULL, $id_etat_perte = NULL) {
  ...
  /* Récupération du compte de liaison */
  $cpt_liaison = ($source == 3)?$DCR["cpt_remb"]:$DCR["cpt_liaison"];
  ...
  if ($DCR['interet_remb_anticipe']> 0) {
    ...
    } else if ($source == 2 || $source == 3 || $source == 4) { // Source = compte lié  et  // Source = compte de garantie
      if ($cpta_debit != NULL) {
...
//  }
  ...
  */
  ...
  if ($source == 1) { // Source = guichet
    ...
    $cptes_substitue["int"]["debit"] = $cpt_liaison;
    if ($source == 4) { // Source = compte de garantie
  ...
  if ($mnt_remb_int > 0) {
    ...
    if ($id_ech_calc == true) {
      ...
      if ($int_cal > 0) {
        $type_oper = get_credit_type_oper(2, 5); //operation Remboursement Interet A Recevoir
  ...
  if ($numrows == 0) { /* Si toutes les échéances sont remboursées */
    ...
    $data_doss = getDossierCrdtInfo($id_doss);
    $info_prod = getProdInfo(" WHERE id = ".$data_doss['id_prod']);
    $flag_mob_lending = $info_prod[0]['is_mobile_lending_credit'];
    $ml_demande_credit = current(getDemandeCredit($data_doss['id_client'], " id_doss = $id_doss"));
...
function rembourseInt($infos_doss, $source, $id_guichet=NULL, $date = NULL, $id_cpte_gar = NULL) {
  ...
  foreach($infos_doss as $id_doss=>$val_doss) {
    ...
    // Ajout dans l'historique
    $myErr = ajout_historique(147, $val_doss['id_client'], $id_doss.'|'.$myErr->param['id_ech'].'|'.$myErr->param['num_remb'], $global_nom_login, date("r"), $comptable, NULL, NULL, $id_doss, $source);
...
// Elle appelle rembourse_montant et s'occupe de l'ajout historique
{
  ...
  foreach($info_doss as $id_doss=>$val_doss) {
    ...
    $myErr = ajout_historique (147, $val_doss['id_client'], $id_doss.'|'.$myErr->param['id_ech'].'|'.$myErr->param['num_remb'], $global_nom_login, date("r"), $comptable_his, NULL, NULL, $id_doss, $source);
    if ($myErr->errCode != NO_ERR) {
      $dbHandler->closeConnection(false);
      return $myErr;
    }
...
function getMontantExigible($id_doss) {
    ...
    $tot_int = 0;
    $tot_pen = 0;
    $whereCond="WHERE (remb='f') AND (id_doss='".$id_doss."') and date_ech <= DATE('".date("Y-m-d")."') ";
...
function get_credit_type_oper($operation, $source=NULL) {
    ...
    if ($operation == 1) { //remb capital
        if ($source == 1)
            return 10;
        else if ($source == 2 || $source == 3 || $source == 4)
            return 10;
...
function getTousEtatCreditAll($en_retard = true) {
    global $dbHandler,$global_id_agence;
    $db = $dbHandler->openConnection();
 ...
 * @return void
 */
function remboursementParLot($limit = 2000) {
    global $dbHandler ,$error;
    global $global_id_agence;
    global $global_nom_login;

    $comptable_his = array();
    $rembourse_auto = array();
    $date_jour = date("d");
    $date_mois = date("m");
    $date_annee = date("Y");
    ...
    bloque_mnt_ech_dossier_collection ($date_max); // Bloque et mettre a jour les montants des echeances des dossiers concernes -> Ticket 693

    $sql = "SELECT main.id_doss, main.id_ech, main.date_ech, main.solde_gar, main.solde_pen, main.solde_int, main.solde_cap, main.id_client, main.cre_id_cpte, main.cpt_liaison, main.etat,main.differe_ech, main.etat_client, main.is_ligne_credit, main.cre_mnt_bloq, main.mnt_bloq_cre FROM (select A.*,case when lag(solde_apres,1) over (partition by id_doss order by id_ech) <= 0 then  row_number() over(partition by id_doss order by id_ech) end as test from (SELECT a.id_doss, id_ech, date_ech, solde_gar, solde_pen, solde_int, solde_cap, b.id_client, b.cre_id_cpte, b.cpt_liaison, b.etat,b.differe_ech, c.etat as etat_client, b.is_ligne_credit, b.cre_mnt_bloq, cpt.mnt_bloq_cre, coalesce(solde_gar,0) + coalesce(solde_pen,0) +coalesce(solde_int,0) + coalesce(solde_cap,0) as total_remb, ROUND(cpt.solde - coalesce(cpt.mnt_bloq,0) - coalesce(cpt.mnt_min_cpte,0) + coalesce(cpt.decouvert_max,0)) as solde_dispo, sum(coalesce(solde_gar,0) + coalesce(solde_pen,0) +coalesce(solde_int,0) + coalesce(solde_cap,0)) OVER (PARTITION BY a.id_doss ORDER BY id_ech ) as cumul, ROUND(cpt.solde - coalesce(cpt.mnt_bloq,0) - coalesce(cpt.mnt_min_cpte,0) + coalesce(cpt.decouvert_max,0)) - sum(coalesce(solde_gar,0) + coalesce(solde_pen,0) +coalesce(solde_int,0) + coalesce(solde_cap,0)) OVER (PARTITION BY a.id_doss ORDER BY id_ech ) as  solde_apres FROM ad_etr a INNER JOIN ad_dcr b on a.id_ag = b.id_ag and a.id_doss = b.id_doss INNER JOIN ad_cli c on b.id_ag = c.id_ag and b.id_client = c.id_client INNER JOIN ad_cpt cpt on b.id_ag = cpt.id_ag and b.cpt_liaison = cpt.id_cpte WHERE b.prelev_auto = TRUE AND (a.date_ech <= '$date_max') AND (a.remb = 'f') AND (b.etat = 5 OR b.etat = 7 OR b.etat = 9 OR b.etat = 13 OR b.etat = 14 OR b.etat = 15) AND ROUND(cpt.solde - coalesce(cpt.mnt_bloq,0) - coalesce(cpt.mnt_min_cpte,0) + coalesce(cpt.decouvert_max,0)) > 0 ORDER BY b.id_doss, a.date_ech) A) main WHERE main.test is null ORDER BY main.id_client ASC, main.id_doss ASC, main.id_ech ASC LIMIT $limit;";
    ...
    while($credit = $result->fetchrow(DB_FETCHMODE_ASSOC)){
        $dataset['data'][] = $credit;
        $dataset['ref'][$credit['id_client']][$credit['id_doss']][] = $credit['id_ech'];
    }

    //Pour chaque échéance passée (trié par ordre chronologique puis par dossier)
    foreach ($dataset['data'] as $credit) {
        ...
        if ($solde_dispo[$cpt_liaison] > 0 && $etat == 2) {
            ...
            if (($mnt_rembours > 0) || ($mnt_rembours >=0 && $differe_ech >0)) {
                if($credit['etat'] != 9){
                    if ($credit['is_ligne_credit'] == 't') {
                        $myErr = rembourse_lcr($id_doss, $mnt_rembours, 2, $comptable_his, NULL, $date_max);
                    } else {

                        $myErr = rembourse($id_doss, $mnt_rembours, 2, $comptable_his, NULL, NULL, NULL, $credit['id_ech'], $date_max, NULL, $DCR, $Produitx, $DEV, $array_credit, $cpta_debit, $cpta_credit_gar, $CPTS_ETAT, $id_etat_perte);
                        if ($myErr->errCode == NO_ERR) {
                            array_push($INFOSREMBAUTO,$myErr->param);
                        }
                    }
                }else{
                    $myErr = recouvrementCreditPerte($id_doss, $mnt_rembours, $comptable_his, $func_sys_remb_credit);
                }

                $last_element = end($dataset['ref'][$credit['id_client']][$credit['id_doss']]);
                if($last_element == $credit['id_ech']){
                    $myErr = ajout_historique (147, $credit['id_client'], $id_doss.'|'.$myErr->param['id_ech'].'|'.$myErr->param['num_remb'], $global_nom_login, date("r"), $comptable_his, NULL);
                    if ($myErr->errCode != NO_ERR) {
                        $dbHandler->closeConnection(false);
                        return $myErr;
                    }
    ...
    $INFOSREMBAUTO['dossier_count'] = $count;
    $dbHandler->closeConnection(true);
    return new ErrorObj(NO_ERR, $INFOSREMBAUTO);
}
function bloque_mnt_ech_dossier_collection ($dateNow){
    global $dbHandler;
    $db = $dbHandler->openConnection();
...
function deboursementCreditAvantGarantie($infos_doss, $mode_debour, $dest_debour, $id_guichet, $prelev_garanti, $func_sys_deb_doss = 125){
    global $dbHandler, $global_id_agence, $global_id_client, $global_nom_login, $global_id_utilisateur;
    global $error;
    global $db;
    // Pour gèrer les garanties de dossier de crédit par un autre client
    ...
    foreach ($infos_doss as $id_doss => $val_doss) {
        ...
        if ($PROD [0] ["percep_frais_com_ass"] == 2) { // Perception des frais, commissions et assurances : APRES deboursement
            if ($comptablemntCrt != NULL) {
                if(!$prelev_garanti){
                    array_unshift($comptable, $comptablemntCrt[1]);
                    array_unshift($comptable, $comptablemntCrt[0]);
                }else{
                    $comptable = array_merge($comptable, $comptablemntCrt);
                }
            }
        ...
        else // Perception des frais, commissions et assurances : AVANT deboursement
        {
            ...
            if (($comptablemntCrt != NULL) && $prelev_garanti) {
                $comptable = array_merge_recursive($comptable, $comptablemntCrt);
            }
        }
...
function array_replace_key(&$array, $old, $new){
    if(array_key_exists($old, $array)){
        $array_keys = array_keys($array);
        $array_keys[array_search($old, $array_keys, true)] = $new;
        $array = array_combine($array_keys, array_values($array));
    }else{
        Signalerreur(__FILE__,__LINE__,__FUNCTION, "Key does not exist");
    }
}
//'search-by-value' may remove values having more than one occurrence
//Fix by implmenting 'search-by-key'
function array_remove_element(&$array, $value){
    array_splice($array, array_search($value, $array), 1);
}
function getNextScreen($screen_order){
    global $global_nom_ecran;
    $screen_order_keys = array_keys($screen_order);
    foreach($screen_order as $key => $value){
        $key_pos = array_search($key, $screen_order_keys);
        $current_pos = array_search($global_nom_ecran, $screen_order_keys);
