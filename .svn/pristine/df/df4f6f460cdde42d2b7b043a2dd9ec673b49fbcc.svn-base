...
function xml_risques_credits($gestionnaire = 0, $export_date, $date_debloc_inf, $date_debloc_sup, $id_prd, $zone, $export_csv = false,$n_agence=null,$n_guichet=null) {
  ...
  //ticket 455
    ...
    if(empty($zone)){
        ...
        $critere_zone = _("Aucun");
    }else{
        $libelle_zone = current(getZone(" id = $zone"));
        $critere_zone = _($libelle_zone["libelle_zone"]);
    }
  ...
  if (isFusion()){
        if ($n_agence> 0){
            $criteres["Niveau agence"]=getLibelLocalisationFenacobu($n_agence);
        }else{
            $criteres["Niveau agence"]="Tous";
        }
      if ($n_guichet> 0){
          $criteres["Niveau guichet"]=getLibelLocalisationFenacobu($n_guichet);
      }else{
          $criteres["Niveau guichet"]="Tous";
      }
  ...
  if($export_date == NULL){
    ...
    if (isFusion()){
        $data = get_risques_credits($gestionnaire, $export_date, $date_debloc_inf, $date_debloc_sup, $id_prd, $zone, $n_agence,$n_guichet);
    }else{
        $data = get_risques_credits($gestionnaire, $export_date, $date_debloc_inf, $date_debloc_sup, $id_prd, $zone );
    }
...
function xml_risques_credits_zone($gestionnaire = 0, $export_date, $date_debloc_inf, $date_debloc_sup, $id_prd, $zone, $export_csv = false,$n_agence=null,$n_guichet=null) {
    /* Génère les données en XML concernant tous les risques de crédits */
    ...
    if ($global_multidevise)
        setMonnaieCourante($global_monnaie);
    //XML
    $document = create_xml_doc("risques_credits", "risques_credits_zone.dtd");
...
//    print_rn($produits);
    ...
    while(list ($zone_key, $zone_value) = each($data["zone"])){
        $zone_node = null;
        while (list ($key, $value) = each($zone_value['detail'])) {
            $zone_node = $produit[$value['cre_etat']][$value['id_prod']]->new_child("zone", "");
            if(strpos($produit[$value['cre_etat']][$value['id_prod']]->node_value(), $zone_key) === false) {
                $zone_node->new_child("libelle_zone", $zone_key);
            }
            $zone_node_ref[$value['cre_etat']][$value['id_prod']] = $zone_node;
            $risque_credit = $zone_node->new_child("risque_credit", "");
            $risque_credit->new_child("num_doss", sprintf("%06d", $key));
            $risque_credit->new_child("duree", $value['duree']);
            $risque_credit->new_child("num_client", makeNumClient($value['id_client']));
            $risque_credit->new_child("nom_client", $value['nom_client']);
    ...
    while (list ($key_etat_cre, $value_etat_cre) = each($etat_credits)) {
        ...
        while (list ($key, $value) = each($produits)) {
            if ($total[$key_etat_cre][$value['id']]['tot_mnt_debloc'] == 0) {
                ...
                $xml_total = $zone_node_ref[$key_etat_cre][$key]->new_child("xml_total", "");
                $xml_total->new_child("tot_mnt_debloc", afficheMontant($total[$key_etat_cre][$value['id']]['tot_mnt_debloc'], false, $export_csv));
                $xml_total->new_child("tot_solde_cap", afficheMontant($total[$key_etat_cre][$value['id']]['tot_solde_cap'], false, $export_csv));
                $xml_total->new_child("tot_solde_int", afficheMontant($total[$key_etat_cre][$value['id']]['tot_solde_int'], false, $export_csv));
                $xml_total->new_child("tot_solde_gar", afficheMontant($total[$key_etat_cre][$value['id']]['tot_solde_gar'], false, $export_csv));
...
function getDonneesRepartitionCredit ($list_agence, $val, $b1, $b2, $duree, $devise = NULL, $list_criteres,$n_agence=null,$n_guichet=null) {
	global $adsys;
  global $global_multidevise,$global_id_agence;
  ...
  if (($devise != NULL) && ($devise != '0') && ($global_multidevise)) {
    ...
    //Liste critere date* 413
      ...
      if (isFusion()){
          $result = get_repartition_credit($duree, $devise, $tranche_data['liste_criteres']["id_gest"] ,$tranche_data['liste_criteres']["Date"],$n_agence,$n_guichet);
      }else{
    ...
    if ($result->errCode != NO_ERR) {
  	...
  	//Liste critere date* 413
      if (isFusion()){
          $result = get_repartition_credit($duree, NULL, $tranche_data['liste_criteres']["id_gest"] ,$tranche_data['liste_criteres']["Date"],$n_agence,$n_guichet);
      }else{
...
//body
 ...
 function xml_balanceportefeuille($gestionnaire = 0, $export_date = NULL, $type_affich = 1, $date_debloc_inf, $date_debloc_sup, $prd, $export_csv = false,$n_agence = null,$n_guichet = null) {
  global $global_multidevise;
  global $global_monnaie;
  ...
  $DATAS = balanceportefeuillerisque($gestionnaire, $export_date, $type_affich, $date_debloc_inf, $date_debloc_sup,$prd,$n_agence,$n_guichet);
  ...
  if (($gestionnaire == 0) || ($gestionnaire == "") || ($gestionnaire == null)) {
      if (isFusion()){
          $criteres = array (
              ...
              _("Date début du déboursement") => date($date_debloc_inf),
              _("Date fin de déboursement") => date($date_debloc_sup),
              _("Niveau agence") => $libelle_agence,
              _("Niveau guichet") => $libelle_guichet
          );
      }else{
          $criteres = array (
              _("Gestionnaire") => _("Tous"),
 ...
 * @return NULL|array
 */
function xml_recouvrement_credit($gestionnaire = 0, $export_date = NULL, $etat = 0, $prd ,$type_affich = 1, $export_csv = false, $export_date_debut = NULL,$n_agence=null,$n_guichet=null)
{
    global $global_multidevise;
    global $global_monnaie;
    ...
    $DATAS = get_rapport_recouvrement_credit_data($gestionnaire, $export_date, $etat, $prd, $export_date_debut,$n_agence,$n_guichet);
