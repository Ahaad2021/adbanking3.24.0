<?php
require_once 'MobLendingMSQPublisher.php';
...
echo "Début de l'execution du fichier failOverMobLendingMSQ.php \n";
...
define('MSQ_QUEUE_NAME_MOB_LENDING', $argv[10]);
define('MSQ_ROUTING_KEY_MOB_LENDING', $argv[11]);

echo "Traitement des messages mobile lending dans la table ad_msq en cours... \n";
$dbHandler = new handleDB();

global $dbHandler;

$typeMsg = BaseMSQMessage::TYPE_MSG_MOB_LENDING;
...
while($datas = $result->fetchrow(DB_FETCHMODE_ASSOC)){
    ...
    try {
        $mlCreditPublisher->init();

        $mlCreditPublisher->process($rawMessage);

        $mlCreditPublisher->shutdown();
...
$deletePublishedMessages = deleteMsgFromFailoverTable(NULL, BaseMSQMessage::STATUT_TRAITEMENT_SUCCES, $typeMsg);
echo "Purge des messages envoyé en succès chez le broker terminé. \n";

echo "Fin de l'execution du fichier failOverMobLendingMSQ.php \n";
 ...
 */
function deleteMsgFromFailoverTable($dataId = NULL, $statut = NULL, $typeMSG = NULL)
{
    global $dbHandler;
