...
require_once 'modules/rapports/xslt.php';
  ...
  $id_client_temp = $_GET['id_client_temp'];
  $CLI = getClientTempDatas($id_client_temp);
  ...
  if ($CLI["statut_juridique"] == 1 || $CLI["statut_juridique"] == 2 || $CLI["statut_juridique"] == 3) {
    $listeGroupSol=getGroupSol($id_client_temp);
  ...
  if ($CLI["statut_juridique"] == 4) {
    ...
    $result = getListeMembresGrpSolTemp($id_client_temp);
...
$objChampsExtras = new HTML_Champs_Extras ($myForm,'ad_cli',$id_client_temp);
$objChampsExtras->buildChampsExtras(getChampsExtrasCLIENTValues($id_client_temp),TRUE);


  $fill = new FILL_HTML_GEN2();
  $fill->addFillClause ("client", "ad_cli_temp");
  $fill->addCondition("client", "id_temp", $id_client_temp);
