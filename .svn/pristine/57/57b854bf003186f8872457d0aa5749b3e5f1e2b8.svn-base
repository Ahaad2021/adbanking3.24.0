...
if (!is_null($client_info_atm)) {
  ...
  //recuperation mouvement
  $mouvements = getMouvementHistData($id_compte_source, $limit);
  ...
  $err = preleveFraisAbonnement('ATM_TSC', $global_id_client, $type_oper_frais, $mnt_fictif, $fonc_mini_statement_api);
  ...
  if ($erreur->errCode === NO_ERR) {
    $erreur = ajout_historique($fonc_mini_statement_api, $id_client, 'Mini statement', $global_nom_login, date("r"));
    if ($erreur->errCode !== NO_ERR) {
