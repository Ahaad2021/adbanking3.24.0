...
elseif ($global_nom_ecran == "Rcq-11" || $global_nom_ecran == "Rcq-12")
{
    ...
    if (isFusion()){
        if(!is_null($_POST['n_agence'])) {
            $n_agence = getLibelLocalisationFenacobu($_POST['n_agence']);
            $criteres_recherche['n_agence'] = $_POST['n_agence'];
        }
        else{
            $n_agence = _("Tous");
        }
        if(!is_null($_POST['n_guichet'])) {
            $n_guichet = getLibelLocalisationFenacobu($_POST['n_guichet']);
            $criteres_recherche['n_guichet'] = $_POST['n_guichet'];
        }
        else{
            $n_guichet = _("Tous");
        }
    ...
    if(isFusion()){
        $criteres = array(
            ...
            _("Niveau agence") => $n_agence,
            _("Niveau guichet") => $n_guichet
        );
    }else {
        $criteres = array(
            _("Date début") => date($date_debut),
