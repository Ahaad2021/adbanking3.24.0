...
/*{{{ Cra-2 : Critères de recherche pour le rapport généraliste */
  ...
  if ($global_nom_ecran_prec == "Cra-1") {
    ...
    $MyPage->setFieldProperties("gest",FIELDP_ADD_CHOICES, $array_uti);
...
//        ));
    ...
    if(isFusion()) {
        ...
        $MyPage->setFieldProperties("n_agence", FIELDP_HAS_CHOICE_TOUS, true);
        $MyPage->setFieldProperties("n_agence", FIELDP_HAS_CHOICE_AUCUN, false);
        $MyPage->setFieldProperties("n_agence", FIELDP_DEFAULT, $SESSION_VARS['DATA_SAUV']['n_agence']);
        //$MyPage->setFieldProperties("n_agence", FIELDP_IS_REQUIRED, true);
        ...
        //AT-150
        // --> Page reloaded Constrution des choix disponibles pour le district
        if (isset($SESSION_VARS['DATA_SAUV']['n_agence']) && isset($SESSION_VARS['DATA_SAUV']['n_guichet'])) {
            $choices = array();
            reset($nivArrayFenacobu);
            while (list(, $value) = each($nivArrayFenacobu)) {
                if ($value['parent'] == $SESSION_VARS['DATA_SAUV']['n_agence'])
                    $choices[$value['id']] = $value['libelle_niveau'];
            }
            $MyPage->setFieldProperties("n_guichet", FIELDP_ADD_CHOICES, $choices);
            $MyPage->setFieldProperties("n_guichet", FIELDP_DEFAULT, $SESSION_VARS['DATA_SAUV']['n_guichet']);
        } else {
//        $MyPage->setFieldProperties("n_guichet", FIELDP_INCLUDE_CHOICES, array(
//            "0" => "[Tous]"
//        ));
...
/*{{{ Cra-3 et Cra-8 : Impression ou export du rapport généraliste */
  ...
  if (isFusion()){
      if ($SESSION_VARS['DATA']['n_agence'] > 0) {
          $list_criteres = array_merge($list_criteres, array(_("Niveau agence") => getLibelLocalisationFenacobu($SESSION_VARS['DATA']['n_agence'])));
      }else{
          $list_criteres = array_merge($list_criteres, array(_("Niveau agence") => "Tous"));
      }
      if ($SESSION_VARS['DATA']['n_guichet'] > 0) {
          $list_criteres = array_merge($list_criteres, array(_("Niveau guichet") => getLibelLocalisationFenacobu($SESSION_VARS['DATA']['n_guichet'])));
      }else{
          $list_criteres = array_merge($list_criteres, array(_("Niveau guichet") => "Tous"));
      }
  }
...
/*{{{ Cra-17 et Cra-18 : Impression concentration clients */
  ...
  if (isset($localisation_main) && $localisation_main > 0){
  	if (isFusion()){
        $concentre_data = getConcentrationClients($tranche_age, $statjuridik, $local, $secteur, null, $localisation_main, $crit_loc,$n_agence,$n_guichet);
        if ($n_agence > 0){
            $list_criteres = array_merge($list_criteres, array (_("Niveau agence ") => getLibelLocalisationFenacobu($n_agence)));
        }else{
            $list_criteres = array_merge($list_criteres, array (_("Niveau agence ") => "Tous"));
        }
        if ($n_guichet > 0){
            $list_criteres = array_merge($list_criteres, array (_("Niveau guichet ") => getLibelLocalisationFenacobu($n_guichet)));
        }else{
            $list_criteres = array_merge($list_criteres, array (_("Niveau guichet ") => "Tous"));
        }
    }else{
    ...
    if(isFusion()){
        $concentre_data = getConcentrationClients($tranche_age, $statjuridik, $local, $secteur, $localisation,null,null,$n_agence,$n_guichet);
        if ($n_agence > 0){
            $list_criteres = array_merge($list_criteres, array (_("Niveau agence ") => getLibelLocalisationFenacobu($n_agence)));
        }else{
            $list_criteres = array_merge($list_criteres, array (_("Niveau agence ") => "Tous"));
