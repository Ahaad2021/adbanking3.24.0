...
function csvChequiers($a_chequiers_print) {
  ...
  foreach ($a_chequiers_print as $id => $tab_comde_chequier) {
  	...
  	if (isset($tab_comde_chequier['n_agence']) && isset($tab_comde_chequier['n_guichet'])){
        $n_agence= $tab_comde_chequier ['n_agence'];
        $n_guichet= $tab_comde_chequier ['n_guichet'];
        $csv .= "\n".$num_complet_cpte.$separator.$nom_cli.$separator.$nbre_carnet.$separator.$n_agence.$separator.$n_guichet.$separator;
    }
    else{
