...
elseif ($global_nom_ecran == "Kra-11") {
      ...
      $utilis_dataset = getLibelUtilisateur();
      ...
      $MyPage->setFieldProperties("gest", FIELDP_ADD_CHOICES, $utilis_dataset);
...
elseif ($global_nom_ecran == "Kra-32" || $global_nom_ecran == "Kra-34") {
        ...
        if (!empty ($prd)) {
          ...
          if ($global_nom_ecran == 'Kra-34') {
          	...
          	//Génération du XSL-FO (grâce au XSLT) et du PDF (grâce à FOP)
          	if (isFusion()){
                $xml = xml_balanceportefeuille($gest, $export_date, $type_affich, $date_debloc_inf1, $date_debloc_sup1, $SESSION_VARS['id_prod'],FALSE,$n_agence,$n_guichet);
            }else{
...
elseif ($global_nom_ecran == "Kra-35") {
	    ...
	    // Date Deboursement- Tri par date debut et date fin de deboursement
	      ...
	      if (isFusion()){
              ...
              $MyPage->addField("n_agence", _("Niveau agence"), TYPC_LSB);
              $MyPage->addField("n_guichet", _("Niveau guichet"), TYPC_LSB);
              $MyPage->setFieldProperties("n_agence", FIELDP_ADD_CHOICES, $arrayDisplay);
              $MyPage->setFieldProperties("n_agence", FIELDP_HAS_CHOICE_TOUS, true);
              $MyPage->setFieldProperties("n_agence", FIELDP_HAS_CHOICE_AUCUN, false);
              //$MyPage->setFieldProperties("n_agence", FIELDP_IS_REQUIRED, true);
              ...
              $MyPage->addJS(JSP_FORM, "jsCodeNivFenacobu", $jsCodeNivFenacobu);
              // --> ajout des champs
              ...
              if (isset($n_agence) && isset($n_guichet)){
                  ...
                  $MyPage->setFieldProperties("n_guichet", FIELDP_ADD_CHOICES, $choices);
                  $MyPage->setFieldProperties("n_guichet", FIELDP_DEFAULT, $n_guichet);
              }
              else{
//        $MyPage->setFieldProperties("n_guichet", FIELDP_INCLUDE_CHOICES, array(
//            "0" => "[Tous]"
//        ));
                  $MyPage->setFieldProperties("n_guichet", FIELDP_HAS_CHOICE_TOUS, true);
                  $MyPage->setFieldProperties("n_guichet", FIELDP_HAS_CHOICE_AUCUN, false);
              }
              //$MyPage->setFieldProperties("n_guichet", FIELDP_IS_REQUIRED, true);
              $MyPage->setFieldProperties("n_agence", FIELDP_JS_EVENT, array("onchange" => "displayNivFenacobu()"));
      ...
      if (isSiege()) {
        ...
        $list_agence[$global_id_agence] = $global_id_agence; //Dans une agence
              ...
              if ($gest == "")
               ...
               //Date filtre 
                ...
                if (isFusion()){
                    if (!empty($n_agence)){
                        $list_criteres = array_merge($list_criteres, array (
                            _("Niveau agence") => getLibelLocalisationFenacobu($n_agence)));
                    }
                    else {
                        $list_criteres = array_merge($list_criteres, array (
                            _("Niveau agence") => _('Tous')));
                    }
                    if (!empty($n_guichet)){
                        $list_criteres = array_merge($list_criteres, array (
                            _("Niveau guichet") => getLibelLocalisationFenacobu($n_guichet)));
                    }
                    else {
                        $list_criteres = array_merge($list_criteres, array (
                            _("Niveau guichet") => _('Tous')));
                    }
                }
              ...
              if ($global_nom_ecran == 'Kra-22') {
                  if (isFusion()){
                      $erreur = getDonneesRepartitionCredit ($list_agence, $val, $b1, $b2, $duree, $devise, $list_criteres,$n_agence,$n_guichet);
                  }else{
...
//                        $html->addTableRefField("gest", _("Gestionnaire"), "ad_uti");
                        ...
                        $html->setFieldProperties("gest", FIELDP_ADD_CHOICES, $utilis_dataset);
...
elseif ($global_nom_ecran == 'Kra-43' || $global_nom_ecran == 'Kra-53') {
  ...
  if ($gest > 0) //Gestionnaire
   $list_criteres = array ();
  $list_criteres = array ("Nombre de clients à afficher" => $nombre_cli,"Encours minimum" => (empty($mnt_min)?' - ':$mnt_min), _("Gestionnaire") => (getLibel("ad_uti", $gest) == "")?_("Tous"):getLibel("ad_uti", $gest));
...
elseif ($global_nom_ecran == 'Kra-46' || $global_nom_ecran == 'Kra-56') {
        ...
        if (isFusion()){
            if (empty($n_agence)){
                $list_criteres[_("Niveau agence")] = "Tous";
            }else{
                $list_criteres[_("Niveau agence")] = getLibelLocalisationFenacobu($n_agence);
            }
            if (empty($n_guichet)){
                $list_criteres[_("Niveau guichet")] = "Tous";
            }else{
                $list_criteres[_("Niveau guichet")] = getLibelLocalisationFenacobu($n_guichet);
            }
        }
        if(isFusion()){
            $DATA = getCreditsPerte($date_deb, $date_fin, $gest, $etat_dossier,$n_agence,$n_guichet);
        }else{
...
elseif ($global_nom_ecran == 'Kra-48' || $global_nom_ecran == 'Kra-58') {
            ...
            if($SESSION_VARS['palier'] == false || $SESSION_VARS['palier'] == NULL){
              ...
              if(isFusion()){
                  if (!empty($n_agence)){
                      $DATA["n_agence"] = $n_agence;
                      $SESSION_VARS['n_agence'] = $n_agence;
                      $SESSION_VARS['liste_criteres'][_("Niveau agence")] = getLibelLocalisationFenacobu($n_agence);
                  }
                  if (!empty($n_guichet)){
                      $DATA["n_guichet"] = $n_guichet;
                      $SESSION_VARS['n_guichet'] = $n_guichet;
                      $SESSION_VARS['liste_criteres'][_("Niveau guichet")] = getLibelLocalisationFenacobu($n_guichet);
                  }
              }
...
//     $MyPage->addTableRefField("gest", _("Gestionnaire"), "ad_uti");
     ...
     foreach($zone_dataset as $key => $value)
       ...
       $MyPage->addField("zone", _("Zone"), TYPC_LSB);
       $MyPage->setFieldProperties("zone", FIELDP_IS_REQUIRED, false);
       $MyPage->setFieldProperties("zone", FIELDP_ADD_CHOICES, $normalized_dataset);
       $MyPage->setFieldProperties("zone", FIELDP_HAS_CHOICE_AUCUN, true);
       $MyPage->setFieldProperties("zone", FIELDP_HAS_CHOICE_TOUS, true);
     ...
     ";
       ...
       $MyPage->addJS(JSP_FORM,"js_reset_droplist",$js);
     //Boutons
     $MyPage->addFormButton(1, 1, "valider", _("Rapport PDF"), TYPB_SUBMIT);
...
//		 }
     ...
     if ($global_nom_ecran == 'Kra-71') {
       ...
       if(!empty($zone)) {
           if (isFusion()){
               $xml = xml_risques_credits($gest, $export_date, $date_debloc_inf1, $date_debloc_sup1, $SESSION_VARS['id_prod'], $zone,false,$n_agence,$n_guichet);
           }else{
               $xml = xml_risques_credits($gest, $export_date, $date_debloc_inf1, $date_debloc_sup1, $SESSION_VARS['id_prod'], $zone);
           }           
           $fichier_pdf = xml_2_xslfo_2_pdf($xml, 'risques_credits.xslt');
       }else{
           if(isFusion()){
               $xml = xml_risques_credits_zone($gest, $export_date, $date_debloc_inf1, $date_debloc_sup1, $SESSION_VARS['id_prod'], $zone,false,$n_agence,$n_guichet);
           }else{
               $xml = xml_risques_credits_zone($gest, $export_date, $date_debloc_inf1, $date_debloc_sup1, $SESSION_VARS['id_prod'], $zone);
           }

           $fichier_pdf = xml_2_xslfo_2_pdf($xml, 'risques_credits_zone.xslt');
       }
       //Message de confirmation + affichage du rapport dans une nouvelle fenêtre
       echo get_show_pdf_html("Gen-13", $fichier_pdf);
     } else
       if ($global_nom_ecran == 'Kra-72') {
         //Génération du CSV grâce à XALAN
         if(!empty($zone)) {
             $xml = xml_risques_credits($gest, $export_date, $date_debloc_inf1, $date_debloc_sup1, $SESSION_VARS['id_prod'], $zone, true);
             $csv_file = xml_2_csv($xml, 'risques_credits.xslt');
         }else{
             $xml = xml_risques_credits_zone($gest, $export_date, $date_debloc_inf1, $date_debloc_sup1, $SESSION_VARS['id_prod'], $zone, true);
             $csv_file = xml_2_csv($xml, 'risques_credits_csv_zone.xslt');
         }
         //Message de confirmation + affichage du rapport dans une nouvelle fenêtre
         if (isset($excel) && $excel == 'Export EXCEL'){
           echo getShowEXCELHTML("Gen-13", $csv_file);
...
elseif ($global_nom_ecran == "Kra-77" || $global_nom_ecran == "Kra-78") {
  ...
  if (!empty ($date_fin)) {
    ...
    if(!empty ($SESSION_VARS['num_client'])){
        ...
        if (isFusion()){
            $DATA = getDoneesRapOctr($SESSION_VARS, 0, $SESSION_VARS['num_client'],$n_agence,$n_guichet);
        }else{
        ...
        //Evolution : ticket 584: groupe solidaires
        if (isFusion()){
            $DATA = getDoneesRapOctr($SESSION_VARS, 0, NULL,$n_agence,$n_guichet);
        }else{
...
{ 
   ...
   //setGlobalIdAgence($agence);
    ...
    if ($global_nom_ecran == 'Kra-96') {
        //Génération du CSV grâce à XALAN      ;
        if (isFusion()){
            $xml = xml_recouvrement_credit($gest, $export_date, $etat, $SESSION_VARS['id_prod'], $type_affich, true, $export_date_debut,$n_agence,$n_guichet) ;
        }else{
        ...
        //Génération du XSL-FO (grâce au XSLT) et du PDF (grâce à FOP)
        if(isFusion()){
            $xml = xml_recouvrement_credit($gest, $export_date, $etat, $SESSION_VARS['id_prod'], $type_affich, false, $export_date_debut,$n_agence,$n_guichet) ;
        }else{
