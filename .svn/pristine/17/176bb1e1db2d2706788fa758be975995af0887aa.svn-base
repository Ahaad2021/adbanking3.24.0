...
function get_data_repartition_epargne($list_agence,$val, $b1, $b2,$date_rapport,$date_debut,$date_fin,$n_agence= null, $n_guichet = null) {
	global $adsys;
	...
	foreach($list_agence as $key_id_ag =>$value) {
		...
		$result = get_repartition_epargne($date_rapport,$date_debut,$date_fin,$n_agence,$n_guichet);
...
function xml_liste_epargne($DATA, $id_prod, $export_csv = false,$date_rapport) {
    ...
    $header_contextuel->new_child("zone", $DATA['zone']);
    $header_contextuel->new_child("agence", $DATA['n_agence']);
    $header_contextuel->new_child("guichet", $DATA['n_guichet']);
	$tot_nb_compte = 0;
	$tot_solde_compte = 0;
	$taille = $DATA['taille'];
	unset($DATA['taille']);
...
function xml_liste_epargne_zone($DATA, $id_prod, $export_csv = false,$date_rapport) {
    global $adsys;
    // XML
    $document = create_xml_doc("liste_epargne", "liste_epargne_zone.dtd");
    set_time_limit(0);
    $root = $document->root();
    gen_header($root, 'EPA-LST',"  ".$date_rapport);
    $header_contextuel = $root->new_child("header_contextuel", "");
...
//      $classe_credit =  $recouvrements_par_classe->new_child("classe_credit", $regroupement_dossiers["classe_credit"]);  //recuperation de libel classecredit
            ...
            foreach ($produit_epargnes as $key1 => $value1) {
                ...
                $titulaires = $value1["titulaires"];
                if (sizeof($titulaires) != 0) {
                    foreach ($titulaires as $zone_def => $titulaires_dataset) {
                        $zone = $prd_eps->new_child("zone", "");
                        $zone->new_child("libelle_zone", $zone_def);
                        $sous_tot_compte = 0;
                        $sous_tot_solde = 0;
                        foreach ($titulaires_dataset as $client_dataset) {
                            $comptes = $zone->new_child("comptes", "");
                            $comptes->new_child("num_client", $client_dataset["id_titulaire"]);
                            $comptes->new_child("nom_client", $client_dataset["nom"]);
                            $comptes_client = $client_dataset["comptes"];
                            $sous_tot_compte += sizeof($comptes_client);
                            if ($export_csv) {
                                foreach ($comptes_client as $key3 => $value3) {
                                    //$compte_csv = $comptes->new_child("compte_csv", "");
                        ...
                        // sous totaux
                        $st = $zone->new_child("sous_total", "");
                        $st->new_child("nb_tot_tit", sizeof($titulaires_dataset));
                        $st->new_child("sous_tot_compte", $sous_tot_compte);
                        $st->new_child("mnt_tot", afficheMontant($sous_tot_solde, true, $export_csv));
                        $tot_nb_compte += $sous_tot_compte;
                        $tot_solde_compte += $sous_tot_solde;
...
function get_data_liste_epargne($list_agence, $id_prod_ep, $type_epargne, $limit,$offset,$date_rapport, $zone,$n_agence = null, $n_guichet = null) {
  global $adsys;
  ...
  foreach($list_agence as $key_id_ag =>$value) {
    ...
    array_merge($array_devise,$devises);
    // Récupère les données
    $result = get_liste_epargne($critere, $limit,$offset,$date_rapport, $type_epargne, $zone,$n_agence,$n_guichet);
    if ($result->errCode != NO_ERR) {
      return $result;
    } else {
      $data = $result->param;
  ...
  if(empty($zone)){
      ...
      $data["zone"] = _($libelle_zone["libelle_zone"]);
  }
  if (!empty($n_agence)){
      $libelle_agence = getLibelLocalisationFenacobu($n_agence);
      $data["n_agence"] = _($libelle_agence);
  }else{
      $data["n_agence"] = _("Tous");
  }
    if (!empty($n_guichet)){
        $libelle_guichet = getLibelLocalisationFenacobu($n_guichet);
        $data["n_guichet"] = _($libelle_guichet);
    }else{
        $data["n_guichet"] = _("Tous");
    }
  return $data;
}
