...
let ${MSQ_QUEUE_NAME_MOB_LENDING:=$(awk -F "=" '/MSQ_QUEUE_NAME_MOB_LENDING/ {print $2}' /usr/share/adbanking/web/adbanking.ini)}
MSQ_ROUTING_KEY_MOB_LENDING=$(awk -F "=" '/MSQ_ROUTING_KEY_MOB_LENDING/ {print $2}' /usr/share/adbanking/web/adbanking.ini)
...
MSQ_QUEUE_NAME_MOB_LENDING="$(echo -e "${MSQ_QUEUE_NAME_MOB_LENDING}" | tr -d '[:space:]')"
MSQ_ROUTING_KEY_MOB_LENDING="$(echo -e "${MSQ_ROUTING_KEY_MOB_LENDING}" | tr -d '[:space:]')"
...
TEST=`return_psql "SELECT count(*) FROM pg_database WHERE datname = lower('$DBNAME') "`
    ...
    then
		...
		START_DATE=`date`
		echo -e "[$START_DATE] Debut Execution du Cron envoie de message Mob Lending..."
		echo -e "Execution du cron pour la publication des messages sur le broker pour la base '$DBNAME'!!"

		php /usr/share/adbanking/web/lib/php-amqplib/failoverMobLendingMSQ.php $DBNAME $datapass $DECRYPTED_MSQ_HOST $MSQ_PORT $DECRYPTED_MSQ_USERNAME $DECRYPTED_MSQ_PASSWORD $MSQ_VHOST $MSQ_EXCHANGE_NAME $MSQ_EXCHANGE_TYPE $MSQ_QUEUE_NAME_MOB_LENDING $MSQ_ROUTING_KEY_MOB_LENDING

        END_DATE=`date`
		echo -e "[$END_DATE] FIN Execution du Cron envoie de message Mob Lending!!"
