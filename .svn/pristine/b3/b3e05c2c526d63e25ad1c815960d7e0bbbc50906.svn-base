...
function getListelocalisationFenacobu($whereCond=null) {
    ...
    $db = $dbHandler->openConnection();
    $sql = "SELECT * FROM adsys_niveau_agence WHERE id_ag=$global_id_agence ";
...
function getNivFenacobuArray($condi=null) {
    ...
    $sql = "SELECT id, libelle_niveau, type_niveau, parent  FROM adsys_niveau_agence where id_ag=$global_id_agence ";
    if ($condi != null){
        $sql .= " AND $condi ";
    }
    $result = $db->query($sql);
...
function getNiVFenacobuNom($id){
    ...
    $sql = "SELECT libelle_niveau from adsys_niveau_agence where id = $id";
