...
/*{{{ Apd-2 : Approbation d'un dossier de crédit */
  ...
  global $adsys, $global_id_agence,$global_id_client,$global_id_utilisateur;
...
//                    }
    ...
    if (isMobileLending()) {
        $premier_credit = isPremierCreditMobileLending($global_id_client);
        if ($premier_credit['premier_credit'] == 't') {
            $SESSION_VARS['infos_prod']['tx_interet'] = 0;
        }
    }
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss)
  {
    ...
    $Myform->setFieldProperties("id_agent_gest".$id_doss,FIELDP_DEFAULT,$SESSION_VARS['infos_doss'][$id_doss]['id_agent_gest']);
    ...
    if (isMobileLending()){
        ...
        if ($flag_mob_lending == 't'){
            $Myform->setFieldProperties("id_agent_gest" . $id_doss, FIELDP_DEFAULT, $global_id_utilisateur);
    ...
    if ($SESSION_VARS['infos_doss'][$id_doss]['etat'] == 7) { // Rééchelonnement/ Moratoire
      ...
      $next_ech = getEcheancier("WHERE id_doss=$id_doss AND remb='f' AND date_ech >  DATE('".date("Y-m-d")."') ");
      $prev_ech = getEcheancier("WHERE id_doss=$id_doss AND id_ech=".($next_ech[0]['id_ech'] - 1));
      $prev_ech = (empty($prev_ech))?array(array('date_ech' => $SESSION_VARS['infos_doss'][$id_doss]['cre_date_debloc'])):$prev_ech;

      if(!empty($next_ech)){
          if(calcNmbreJr(date("Y/m/d"), $prev_ech[0]['date_ech'])){
              $differe_jours = calcNmbreJr(date("Y/m/d"), $prev_ech[0]['date_ech']);
              $prorata = getProrata($id_prod, $next_ech[0]['mnt_int'], $nbr_echeances_min, $differe_jours, $differe_ech, NULL, $id_doss);
          }
      }else{
          $prorata = 0;
      }
      ...
      $SESSION_VARS['infos_doss'][$id_doss]['mnt_reech'] = $MNT_EXIG["int"] + $MNT_EXIG["pen"] + $prorata;
...
/*{{{ Apd-3 : Affichage de l'échéancier */
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    if ($val_doss["last_etat"] == 1) { // L'état du dossier est En attente de décision
      ...
      if (isMobileLending()) {
          $info_prod = getProdInfo(" WHERE id = " . $id_prod);
          $flag_mob_lending = $info_prod[0]['is_mobile_lending_credit'];
          if ($flag_mob_lending == 't') {
              $SESSION_VARS['infos_doss'][$id_doss]["details_motif"] = ${'details_motif' . $id_doss};
          }
      }
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    ...
    foreach($val_doss['DATA_GAR'] as $key=>$value ) {
      if ($value['type'] == 1) { // Garanties numéraires
        ...
        if (($value['id_gar'] != NULL) && $prelev_garantie)
...
/*{{{ Apd-5 : Confirmation approbation d'un dossier de crédit */
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    ...
    if ($val_doss['last_etat'] == 1) { // dossier en attente d'approbation
      ...
      if (isMobileLending()){
          $info_prod = getProdInfo(" WHERE id = ".$val_doss["id_prod"]);
          $flag_mob_lending = $info_prod[0]['is_mobile_lending_credit'];
          if ($flag_mob_lending == 't'){
              $DATA[$id_doss]["details_motif"] = $val_doss["details_motif"];
          }
      }
    ...
    $prelev_garantie = ($SESSION_VARS['infos_prod']['prelev_garanti'] == '1')?true:false;;
    $myErr = approbationCredit($DATA, $prelev_garantie);
