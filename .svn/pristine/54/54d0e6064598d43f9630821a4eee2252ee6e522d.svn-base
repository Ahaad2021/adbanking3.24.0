...
//-----------------------------------------------------------------
	...
	$dev = $SESSION_VARS['devise'];
        ...
        // $type_imprimante = $SESSION_VARS['type_imprimante'];
  ...
  if ($erreur->errCode == NO_ERR) {
    ...
    if (isset($CHANGE)) {
      ...
      printRecuChange ($erreur->param['id'], $SESSION_VARS["mnt"], $cpteSource["devise"], $SESSION_VARS["recu_change"]["source_achat"], $SESSION_VARS["change"]["cv"], $SESSION_VARS["change"]["devise"], $SESSION_VARS["change"]["comm_nette"],$SESSION_VARS["change"]["taux"],$SESSION_VARS["change"]["reste"],$SESSION_VARS["recu_change"]["dest_vente"],$SESSION_VARS["change"]["dest_reste"],$SESSION_VARS["envoi_reste"], $listTypesBilletArr, $valeurBilletArr, $global_langue_rapport, $total_billetArr, $hasBilletageChange, "retrait");
    }

    create_report_tag("epargne",array($report_backup_path => array("num_client" => $global_id_client,"num_transactions" => $erreur->param["id"], "type_recu" => "retrait", "id_ag" => $global_id_agence, 'montant' => $SESSION_VARS['mnt'])));
