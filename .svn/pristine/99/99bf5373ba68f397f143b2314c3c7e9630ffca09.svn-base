...
//---------------------------------------------------------------------
  ...
  if ($SESSION_VARS["TypeTransfert"] == 1) {
    $montant_remboursement = $mnt_reel;
    // Montant des frais de transfert
    ...
    $montant_remboursement -= $frais_transfert;
    ...
    $montant_sms = getPreleveFraisSMS($erreur->datas);
...
*/
  ...
  elseif ($SESSION_VARS["TypeTransfert"] == 2) { // transfert sur le compte d'un autre client de la banque
    ...
    if ($InfoClientDestination == NULL) {
      ...
      $montant_remboursement = $montant_remboursement - $frais_transfert;
      // Compte de prélèvement des frais
      if ($SESSION_VARS['frais_transfert']['type_cpte_preleve'] == 1)
        $cpte_preleve = $SESSION_VARS["NumCpteSource"];
      elseif($SESSION_VARS['frais_transfert']['type_cpte_preleve'] == 2)
...
/*Pending en attendant evol pour les salariés/ commercants
      if(isMobileLending()) {
          //EVOL -- MOBILE LENDING
          $montant_remboursement = $montant_remboursement - $mnt_frais_attente;
          $rembourse_erreur = rembourseMontantDepot($SESSION_VARS["num_client"], $montant_remboursement);
    ...
    if(isMobileLending()) {
        //defines the start of each 'comptable' boundary
        $limits = array();
        foreach ($erreur->datas as $index => $dataset) {
            if (in_array($SESSION_VARS["NumCpteSource"], $dataset)) {
                $limits[] = $index;
            }
        }
        $limits[] = count($erreur->datas);

        $temp = array();
        //consolidates the 'çomptable' array as per the boundaries
        for ($pos = 0; $pos < count($limits) - 1; $pos++) {
            $temp[] = array_slice($erreur->datas, $limits[$pos], $limits[$pos + 1] - $limits[$pos]);
        }

        $montant_sms = array();
        foreach ($temp as $arr_comptable) {
            $montant_sms[] = getPreleveFraisSMS($arr_comptable);
        }
    }

    //prélèvement des frais en attente s'il ya lieu
    foreach($SESSION_VARS["DATA"] as $key_data => $value) {
	    //prélèvement des frais en attente si solde_disponible > montant_frais
	    $prelevement_frais = false;
        ...
        $SESSION_VARS["DATA"][$key_data]["mnt_frais_attente"] = $montant_frais;
    }
...
/*Pending en attendant evol pour les salariés/ commercants
        if(isMobileLending()) {
            $next_index = 0;
            foreach ($SESSION_VARS["DATA"] AS $arr_key => $value) {
                $montant_remboursement = $value["mnt_dest"];
                $montant_remboursement = $montant_remboursement - $value["mnt_frais"];
                $montant_remboursement = $montant_remboursement - $value["mnt_frais_attente"];
                $montant_remboursement -= $montant_sms[$next_index];

                //EVOL -- MOBILE LENDING
                $rembourse_erreur = rembourseMontantDepot($value["id_client"], $montant_remboursement);
