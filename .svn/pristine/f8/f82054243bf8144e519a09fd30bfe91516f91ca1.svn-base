...
require_once 'ad_ma/app/models/Remboursement.php';
...
{    
    ...
    $date_compta = $ComptaObj->getExercicesComptables($global_remote_id_exo);
    $date_batch = date_create(str_replace('/', '-',pg2phpDate(get_last_batch($global_id_agence))));
    date_add($date_batch, date_interval_create_from_date_string('1 day'));
    $date_batch_timestamp = strtotime(date_format($date_batch, 'd-m-Y'));
    $date_compta_debut = strtotime($date_compta[0]['date_deb_exo']);
    $date_compta_fin = strtotime($date_compta[0]['date_fin_exo']);

    if (!(($date_batch_timestamp >= $date_compta_debut) && ($date_batch_timestamp <= $date_compta_fin))) {
        $html_err = new HTML_erreur(_("Echec du dépôt en déplacé sur un compte."));
        $html_err->setMessage(_("La date donnée n'est pas valide"));
    ...
    if ($SESSION_VARS["type_depot"] == 1)
    {
        ...
        $montant_remboursement = $SESSION_VARS["mnt"]; // EVOL ML-76
        ...
        try
        {
            ...
            //mouvement des comptes avec gestion des frais d'opérations sur compte s'il y lieu
            $erreur_remote = Depot::depotCpteRemote($pdo_conn, $global_remote_id_agence, $global_id_guichet, $SESSION_VARS["NumCpte"], $SESSION_VARS["mnt"], $InfoProduit, $InfoCpte, $data, $type_depot, $CHANGE, null, null, $montant_remboursement);
        ...
        if ($erreur_remote->errCode == NO_ERR && $erreur_local->errCode == NO_ERR) // no errors
        {
            ...
            try
            {
                ...
                $montant_remboursement -= $mnt_frais_attente;
                /*
                if(isMobileLending()) {
                    //EVOL ML-76 (Amélioration pour le remboursement automatique lors d'un dépôt)
                    $rembourse_obj = new Remboursement($pdo_conn, $global_remote_id_agence);
                    $erreur_rembourse_remote = $rembourse_obj->rembourseMontantDepot($pdo_conn, $montant_remboursement, $global_remote_id_agence, $global_remote_id_client, $global_id_guichet);

                    if ($erreur_rembourse_remote->errorCode != NO_ERR) {
                        $html_err = new HTML_erreur(_("Echec du remboursement."));
                        $html_err->setMessage(_("Erreur") . " : " . $error[$erreur_rembourse_remote->errCode] . "<br/>" . _("Paramètre") . " : " . $erreur_rembourse_remote->param);
    ...
    { 
        ...
        try
        {
            ...
            $erreur = $EpargneObj->receptionCheque($data, $InfoCpte, $InfoProduit, $SESSION_VARS["mnt"], $creditDirectSaufBonneFin, $commissionSurCreditDirect, $CHANGE, $montant_remboursement);
    ...
    {        
        ...
        try
        {
            ...
            // effectuer virement :       
            $erreur = $EpargneObj->receptionVirement($data, $InfoCpte, $InfoProduit, null,  null, 75, $CHANGE, $montant_remboursement);
