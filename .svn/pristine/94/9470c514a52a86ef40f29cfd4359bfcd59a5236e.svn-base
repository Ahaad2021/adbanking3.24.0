	...
	require_once 'lib/misc/excel.php';
 	...
 	/*{{{ Era-5 : Comptes inactifs : choix du seuil en jours */
 	    ...
 	    if ($global_nom_ecran == "Era-5") {
            ...
            $jquery_src = "<script src=\"/lib/misc/js/lib/jquery.min.js\" type=\"text/javascript\"></script>";
            $html->addHTMLExtraCode("xtHTML", $jquery_src);
			...
			$gestionnaire = getLibelUtilisateur();
			$html->addField("gest", _("Gestionnaire"), TYPC_LSB);
			$html->setFieldProperties("gest", FIELDP_ADD_CHOICES, $gestionnaire);
 	    ...
 	    if ($global_nom_ecran == "Era-7") {
			...
			$html->setFieldProperties("gest", FIELDP_ADD_CHOICES, $gestionnaire);
 	        ...
 	        if(isFusion()) {
                ...
                $html->addField("n_agence", _("Niveau agence"), TYPC_LSB);
                $html->addField("n_guichet", _("Niveau guichet"), TYPC_LSB);
                $html->setFieldProperties("n_agence", FIELDP_ADD_CHOICES, $arrayDisplay);
                $html->setFieldProperties("n_agence", FIELDP_HAS_CHOICE_TOUS, true);
                $html->setFieldProperties("n_agence", FIELDP_HAS_CHOICE_AUCUN, false);
                //$html->setFieldProperties("n_agence", FIELDP_IS_REQUIRED, true);
                ...
                $html->addJS(JSP_FORM, "jsCodeNivFenacobu", $jsCodeNivFenacobu);
                // --> ajout des champs
                ...
                if (isset($n_agence) && isset($n_guichet)) {
                    ...
                    $html->setFieldProperties("n_guichet", FIELDP_ADD_CHOICES, $choices);
                    $html->setFieldProperties("n_guichet", FIELDP_DEFAULT, $n_guichet);
                } else {
//        $html->setFieldProperties("n_guichet", FIELDP_INCLUDE_CHOICES, array(
//            "0" => "[Tous]"
//        ));
                    $html->setFieldProperties("n_guichet", FIELDP_HAS_CHOICE_TOUS, true);
                    $html->setFieldProperties("n_guichet", FIELDP_HAS_CHOICE_AUCUN, false);
                }
                //$html->setFieldProperties("n_guichet", FIELDP_IS_REQUIRED, true);
                $html->setFieldProperties("n_agence", FIELDP_JS_EVENT, array("onchange" => "displayNivFenacobu()"));
 	...
 	/*{{{ Era-8 et Era-28 : DAT arriant à échéance : impression ou export csv */
 	    ...
 	    if ($global_nom_ecran == 'Era-8' || $global_nom_ecran == "Era-28") {
 	        ...
 	        if ($gest == "")
 	            $gest = 0;
 	        if (isFusion()){
                $lignesDATEcheance = getLignesDATEcheance($periode, $exclusif, $gest,$date_debut,$date_fin,$n_agence,$n_guichet);
			}else{
 	        ...
 	        if ($global_nom_ecran == "Era-28") {
 	            ...
 	            if ($lignesDATEcheance != NULL) {
 	            	if (isFusion()){
                        $xml = xml_DAT_echeance($lignesDATEcheance, array(_("Periode") => $SESSION_VARS["periode_rapport_DAT_echeance"][$periode] == null ? _("Tous") : $SESSION_VARS["periode_rapport_DAT_echeance"][$periode], _("Gestionnaire") => (getLibel("ad_uti", $gest) == "") ? _("Tous") : getLibel("ad_uti", $gest), _("Niveau agence") => (getLibelLocalisationFenacobu($n_agence) == "") ? _("Tous") : getLibelLocalisationFenacobu($n_agence), _("Niveau guichet") => (getLibelLocalisationFenacobu($n_guichet) == "") ? _("Tous") : getLibelLocalisationFenacobu($n_guichet)), true);
					}else{
 	            ...
 	            if ($global_nom_ecran == "Era-8") {
 	                ...
 	                if ($lignesDATEcheance != NULL){
                        if (isFusion()){
                            $xml = xml_DAT_echeance($lignesDATEcheance, array(_("Periode") => $SESSION_VARS["periode_rapport_DAT_echeance"][$periode] == null ? _("Tous") : $SESSION_VARS["periode_rapport_DAT_echeance"][$periode], _("Gestionnaire") => (getLibel("ad_uti", $gest) == "") ? _("Tous") : getLibel("ad_uti", $gest), _("Niveau agence") => (getLibelLocalisationFenacobu($n_agence) == "") ? _("Tous") : getLibelLocalisationFenacobu($n_agence), _("Niveau guichet") => (getLibelLocalisationFenacobu($n_guichet) == "") ? _("Tous") : getLibelLocalisationFenacobu($n_guichet)));
                        }else{
 	...
 	/*{{{ Era-9 : Etat général des comptes de base à solde non nul : personnalisation */
 	    ...
 	    if ($global_nom_ecran == 'Era-9') {
			...
			if(isFusion()) {
                ...
                $myForm->addField("n_guichet", _("Niveau guichet"), TYPC_LSB);
                $myForm->setFieldProperties("n_agence", FIELDP_ADD_CHOICES, $arrayDisplay);
                $myForm->setFieldProperties("n_agence", FIELDP_HAS_CHOICE_TOUS, true);
                $myForm->setFieldProperties("n_agence", FIELDP_HAS_CHOICE_AUCUN, false);
...
//            "0" => "[Tous]"
//        ));
                    $myForm->setFieldProperties("n_guichet", FIELDP_HAS_CHOICE_TOUS, true);
                    $myForm->setFieldProperties("n_guichet", FIELDP_HAS_CHOICE_AUCUN, false);
                }
 	...
 	/*{{{ Era-10 et Era-29 : Etat général des comptes de base à solde non nul : impression ou export CSV */
 	    ...
 	    if ($global_nom_ecran == 'Era-10' || $global_nom_ecran == 'Era-29') {
 	        ...
 	        if (isFusion()){
                $DATA = getSoldeCpteEpargne($prodEpargne, $idmin, $idmax,$date_deb,$n_agence, $n_guichet);
                $produit = getProdEpargne($prodEpargne);
                if ($n_agence > 0){
                ...
                $CRITERE = array (_("Produit d'épargne ") => $produit['libel'], _("ID client minimum") => $idmin, _("ID client maximum") => $idmax,_("Niveau agence") =>  $n_agence_libel,_("Niveau guichet") => $n_guichet_libel);
			}else{
 	...
 	/*{{{ Era-11 : Solde des comptes de base mouvementés dans la journée : personnalisation */
 	    ...
 	    if ($global_nom_ecran == 'Era-11') {
			...
			$gestionnaire = getLibelUtilisateur();
			$myForm->addField("gest", _("Gestionnaire"), TYPC_LSB);
			$myForm->setFieldProperties("gest", FIELDP_ADD_CHOICES, $gestionnaire);
...
//        ));
	...
	if ($critere == "loc" && $Data_agence['identification_client'] == 2){//AT-33/AT-77 - dans le contexte de Rwanda
		...
		if (isFusion()){
            $tranche_data = get_data_repartition_epargne($list_agence, $val, $b1, $b2,$date_rapport,$date_debut,$date_fin,$n_agence,$n_guichet);
		}else{
 	...
 	/*{{{ Era-18 et Era-33 : Créditeurs les plus importants : impression ou export csv */
 	    ...
 	    if ($global_nom_ecran == 'Era-18' || $global_nom_ecran == 'Era-33') {
 	        ...
 	        if (($global_multidevise) && ($devise != '0')) {
                if (isFusion()){
                    $DATA = getListeCompteSupamin($montantmin, $devise, $gest,$n_agence, $n_guichet);
                    if ($n_agence > 0){
                        $n_agence_libel = getLibelLocalisationFenacobu($n_agence);
					}else{
                        $n_agence_libel = "Tous";
                ...
                if (isFusion()){
                    $DATA = getListeCompteSupamin($montantmin, NULL, $gest,$n_agence, $n_guichet);
                    if ($n_agence > 0){
                        $n_agence_libel = getLibelLocalisationFenacobu($n_agence);
                    }else{
                        $n_agence_libel = "Tous";
 	        ...
 	        if ($global_nom_ecran == 'Era-18') {
 	            //Génération du XSL-FO (grâce au XSLT) et du PDF (grâce à FOP)
				if (isFusion()){
                    $xml = xml_liste_CompteSupamin($DATA, $devise, array (_("Gestionnaire") => $list_criteres = (getLibel("ad_uti", $gest) == "") ? _("Tous") : getLibel("ad_uti", $gest), _("Niveau agence")=> $n_agence_libel ,_("Niveau guichet")=> $n_guichet_libel), $list_criteres);
				}else{
 	            ...
 	            if ($global_nom_ecran == 'Era-33') {
 	                //Génération du csv grâce à XALAN
					if(isFusion()){
                        $xml = xml_liste_CompteSupamin($DATA, $devise, array (_("Gestionnaire") => $list_criteres = getLibel("ad_uti", $gest),_("Niveau agence")=> $n_agence_libel ,_("Niveau guichet")=> $n_guichet_libel), $list_criteres, true);
					}else{
...
/*}}}*/
    ...
    unset($SESSION_VARS['critere_ep']);
        ...
        $normalized_dataset = array();

        foreach($zone_dataset as $key => $value)
            $normalized_dataset[$value["id"]] = $value["libelle_zone"];

        $myForm->addField("zone", _("Zone"), TYPC_LSB);
        $myForm->setFieldProperties("zone", FIELDP_IS_REQUIRED, false);
        $myForm->setFieldProperties("zone", FIELDP_ADD_CHOICES, $normalized_dataset);
        $myForm->setFieldProperties("zone", FIELDP_HAS_CHOICE_AUCUN, true);
        $myForm->setFieldProperties("zone", FIELDP_HAS_CHOICE_TOUS, true);
...
//		}
		...
		$JS = "\n function createOption(dropdown, text, value) {
				...
				function getProduitsEpargnesByType(type_epargne, produit_epargne)
					{
						var s_produit_epargne = '".addslashes($choix_ep)."';
					...
					$('[name = HTML_GEN_LSB_zone] option:selected').prop('selected', false);
					$('[name = HTML_GEN_LSB_zone] option').each(function(index, node){
						if($(node).text() == '[Aucun]'){
							//value of this option is overwitten to due to a deeply nested bug
							$(node).attr('value', -1);
...
//		$myForm->setFieldProperties("critere_ep", FIELDP_ADD_CHOICES, $choix_ep);
	...
	if(!isset($SESSION_VARS['zone'])){
		$SESSION_VARS['zone'] = $zone;
	}

	if (!isset($SESSION_VARS['n_guichet'])){
        if(!isset($SESSION_VARS['n_agence'])){
            $SESSION_VARS['n_agence'] = $n_agence;
        }
    }
    if (isset($SESSION_VARS['n_guichet'])){
        if(!isset($SESSION_VARS['n_guichet'])){
            $SESSION_VARS['n_guichet'] = $n_guichet;
        }
    }
	...
	if( !isset($SESSION_VARS['nombre_lignes'])) {
        if (isFusion()){
            $SESSION_VARS['nombre_lignes'] = getNbreComptesEpargne($SESSION_VARS['critere_ep'], $SESSION_VARS['type_epargne'],$date_rapport,$SESSION_VARS['n_agence'],$SESSION_VARS['n_guichet']);
        }else{
	...
	$tranche_data = get_data_liste_epargne($list_agence, $SESSION_VARS['critere_ep'], $SESSION_VARS['type_epargne'], $SESSION_VARS['nbre_ligne_rapport'],$offset,$SESSION_VARS['date_rapport'], $SESSION_VARS['zone'],$SESSION_VARS['n_agence'],$SESSION_VARS['n_guichet']);
	...
	if ($global_nom_ecran == 'Era-44') {
		//Génération du XSL-FO (grâce au XSLT) et du PDF (grâce à FOP)
		if(!empty($SESSION_VARS['zone'])){
            $xml = xml_liste_epargne($tranche_data, $SESSION_VARS['critere_ep'],FALSE,$SESSION_VARS['date_rapport']);
            $fichier_pdf = xml_2_xslfo_2_pdf($xml, "liste_epargne.xslt");
		}else{
            $xml = xml_liste_epargne_zone($tranche_data, $SESSION_VARS['critere_ep'],FALSE,$SESSION_VARS['date_rapport']);
            $fichier_pdf = xml_2_xslfo_2_pdf($xml, "liste_epargne_zone.xslt");
		}
        ...
        if(!empty($SESSION_VARS['zone'])) {
            ...
            $xml = xml_liste_epargne_zone($tranche_data, $SESSION_VARS['critere_ep'], TRUE, $SESSION_VARS['date_rapport']);
            $new_xml = str_replace("<compte_numeros>", "", $xml);
            $new_xml = str_replace("</compte_numeros>", "", $new_xml);
            $new_xml = str_replace("<compte_soldes>", "", $new_xml);
            $new_xml = str_replace("</compte_soldes>", "", $new_xml);

            $csv_file = xml_2_csv($new_xml, "liste_epargne_csv_zone.xslt");
		}
//		if ($arreter){
...
//        ));
         ...
         /*{{{ Era-55 et  Era-56 : SLD - Impression ou export Rapport INVENTAIRE DE DEPOT */
             ...
             if ($global_nom_ecran == 'Era-55' || $global_nom_ecran == 'Era-56') {
                 ...
                 if (isFusion()){
                     if(isset($n_agence)){
                         $libelle_agence = getLibelLocalisationFenacobu($n_agence);
                         $critere[_("Niveau agence")]=$libelle_agence;
                     }
                     if(isset($n_guichet)){
                         $libelle_guichet = getLibelLocalisationFenacobu($n_guichet);
                         $critere[_("Niveau guichet")]=$libelle_guichet;
                     }
                 }
                 unset($SESSION_VARS['prodEpa']);
                 ...
                 //recupération des données à afficher
                 if (isFusion()){
                     $DATA = get_rapport_inventaire_depot($where,$date_deb,$date_fin,$data_agc['identification_client'],$n_agence,$n_guichet);
                 }else{
