...
/**
 * Gestion des operations en deplacees via agent
 * @package Agency Banking
 */
...
// God-1 : choix Agence
if ($global_nom_ecran == "God-1"){
  global $global_nom_login, $tableau_border;

//-------------------------------- ECRAN God-1 : Opération en déplacé ------------------------------------------
  ...
  $html->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, 'God-2');
    ...
    if ((strstr($global_nom_ecran_prec, "God-1"))) {
      if (isset($_POST['agence_cliente'])) {
        ...
        if ($agenceCo != null){
          ...
          $html_err->addButton("BUTTON_OK", 'God-1');
          $html_err->buildHTML();
...
//-------------------------------- ECRAN God-2 : Opération en déplacé ------------------------------------------
    ...
    $html->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, 'God-3');
    $html->setFormButtonProperties("back", BUTP_PROCHAIN_ECRAN, 'God-1');
...
// God-3 : Menu operation en deplacee via agent
elseif ($global_nom_ecran == "God-3") {
...
//-------------------------------- ECRAN God-3 : Menu Opération en déplacé --------------------------------
  ...
  try {
    ...
    if ($clientDetailsArr == NULL) { //Si le client n'existe pas
      ...
      $erreur->addButton(BUTTON_OK, "God-2");
      $erreur->buildHTML();
      echo $erreur->HTML_code;
    } else { //Si le client existe
      // Enregistrement l'accès au menu
      ajout_historique(771, NULL, 'Opération en déplacé : ' . $global_remote_id_agence . '-' . $global_remote_id_client, $global_nom_login, date("r"), NULL);
      ...
      if ($agence_statut == 1) {
        if (isset($ListeComptes) && count($ListeComptes) > 0) {
          ...
          if ($display_retrait_link) {
            $MyMenu->addItem(_("Retrait en déplacé via agent"), "$PHP_SELF?m_agc=" . $_REQUEST['m_agc'] . "&prochain_ecran=$ecran_retrait", $fonction_retrait, "$http_prefix/images/retrait.gif", "1");
          ...
          $MyMenu->addItem(_("Dépôt en déplacé via agent"), "$PHP_SELF?m_agc=" . $_REQUEST['m_agc'] . "&prochain_ecran=$ecran_depot", $fonction_depot, "$http_prefix/images/depot.gif", "2");
          $MyMenu->addItem(_("Consultation d'un compte en déplacé via agent"), "$PHP_SELF?m_agc=".$_REQUEST['m_agc']."&prochain_ecran=Ccd-1", 788, "$http_prefix/images/consult_cpt.gif","3");
        }
      } else {
